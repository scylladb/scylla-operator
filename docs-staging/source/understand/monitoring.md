...
|------|---------------------|
| **`Platform`** (default) | Full set of dashboards: overview, CQL, detailed, advanced, OS, keyspace, and Alternator. Use when you manage the Kubernetes nodes and want infrastructure-level visibility. Also includes a ScyllaDB Manager dashboard. |
| **`SaaS`** (deprecated) | A single ScyllaDB overview dashboard. Suitable when you do not control the underlying infrastructure. Deprecated and will be removed in a future version; use `Platform` instead. |
