...
```bash
podman run -it --pull=Always --rm -v="$( pwd )/:/go/$( go list -m )/:Z" --workdir="/go/$( go list -m )/docs" -p 5500:5500 quay.io/scylladb/scylla-operator-images:uv-0.10 bash -euExo pipefail -O inherit_errexit -c 'make setup && make preview'
```
...
```bash
podman run -it --pull=Always --rm -v="$( pwd )/:/go/$( go list -m )/:Z" --workdir="/go/$( go list -m )/docs" -p 5500:5500 quay.io/scylladb/scylla-operator-images:uv-0.10 bash -euExo pipefail -O inherit_errexit -c 'make update'
```
