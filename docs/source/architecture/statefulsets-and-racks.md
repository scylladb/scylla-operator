...
:::{important}
You cannot remove an arbitrary node from the middle of a rack's ordinal range by changing the `members` count. Scaling always operates on the tail of the StatefulSet. To remove a specific node (for example, ordinal 1 out of 0, 1, 2), use node replacement instead — see [Replacing nodes](../operating/replacing-nodes.md).
:::
...
To diagnose, check the logs of the failing pod and its events. If the problem is with the new configuration, fix the spec and the rollout resumes automatically. If the pod is stuck for other reasons (crash loop, resource issues), see [Node not starting](../troubleshooting/node-not-starting.md).

For log-level changes on a stuck cluster where a rolling restart is not feasible, see [Changing log level](../troubleshooting/changing-log-level.md).

### Deleting a StatefulSet

Deleting a StatefulSet with the default cascade policy also deletes all its pods (but not PVCs). If the Operator is running, it immediately recreates the StatefulSet, which recreates the pods. Using `--cascade=orphan` preserves the running pods — this is used during advanced recovery procedures such as [Recovering from a failed replace](../troubleshooting/recovering-from-failed-replace.md).
...
- [Scaling](../operating/scaling.md) — how-to guide for scaling operations.
- [Replacing nodes](../operating/replacing-nodes.md) — how-to guide for node replacement.
- [Changing log level](../troubleshooting/changing-log-level.md) — emergency changes when a rollout is stuck.
