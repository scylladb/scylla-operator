...
Performance tuning is enabled by default **when you create a corresponding [NodeConfig](../installation/nodeconfigs-old.md) for your nodes**.
...
This is executed right after the tuning is enabled using a [NodeConfig](../installation/nodeconfigs-old.md)

## ScyllaDB Pods

When a [ScyllaCluster](../tutorials/day0/create-cluster.md) Pod is created (and performance tuning is enabled), the Pod initializes but waits until ScyllaDB Operator runs an on-demand Job that will configure the host and the ScyllaDB process accordingly (e.g. spreading IRQs across other CPUs).
