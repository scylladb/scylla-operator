...
To use your own TLS certificates instead of operator-managed ones, set `servingCertificate.type: UserManaged` on the ScyllaCluster. See [TLS and certificate management](../architecture/security.md).
...
- [External access](external-access.md) — connecting from outside the Kubernetes cluster.
