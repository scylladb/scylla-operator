...
|---|------|----------------|-------|
| 1 | **NodeConfig deployed** | A `NodeConfig` resource is applied and `Available=True` in each cluster running ScyllaDB. NodeConfig configures disk setup, kernel parameters, and performance tuning — without it, ScyllaDB runs on untuned nodes with default kernel settings. | [Node configuration](configure-nodes.md) |
| 2 | **Dedicated node pool** | ScyllaDB runs on isolated nodes with label `scylla.scylladb.com/node-type=scylla` and taint `scylla-operator.scylladb.com/dedicated=scyllaclusters:NoSchedule`. No other workloads are scheduled on these nodes. | [Set up dedicated node pools](set-up-dedicated-node-pools.md) |
| 3 | **Performance tuning co-located** | NodeConfig `placement` targets the same nodes as the ScyllaCluster / ScyllaDBCluster placement. Both use the same label selector and toleration. If they target different node sets, tuning does not apply to ScyllaDB pods. | [Set up dedicated node pools](set-up-dedicated-node-pools.md) |
| 4 | **`fs.nr_open` set** | The sysctl `fs.nr_open` is set to a high value (e.g., `1073741816`) via NodeConfig. This defines the ceiling for `RLIMIT_NOFILE`, which the Operator raises on each ScyllaDB process. Without it, ScyllaDB may fail to open files under heavy load. | [Configure nodes](configure-nodes.md) |
...
| 8 | **CPU pinning active** | The kubelet uses `cpuManagerPolicy: static`. The ScyllaDB pod has Guaranteed QoS class (requests equal limits for all containers). ScyllaDB starts with `--overprovisioned=0`. | [Configure CPU pinning](configure-cpu-pinning.md) |
| 9 | **XFS online discard enabled** | The `discard` mount option is set on ScyllaDB data volumes via NodeConfig's `unsupportedOptions`. This enables SSD TRIM for consistent write performance. | [Configure nodes](configure-nodes.md) |
...
- [Deploy a multi-DC cluster](deploy-multi-dc-cluster.md) — creating a ScyllaDBCluster.
- [Configure nodes](configure-nodes.md) — disk setup, sysctls, and performance tuning.
- [Configure CPU pinning](configure-cpu-pinning.md) — configuring CPU exclusivity.
