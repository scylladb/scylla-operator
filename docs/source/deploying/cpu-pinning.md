...
5. The sidecar ignition mechanism blocks ScyllaDB startup until tuning completes (see [Ignition](../architecture/ignition.md)).
...
- [Dedicated node pools](dedicated-node-pools.md) — setting up nodes with the correct kubelet configuration.
- [Node configuration](node-configuration.md) — NodeConfig resource for disk and kernel tuning.
- [Deploying a single-DC cluster](single-dc-cluster.md) — setting resource requests and limits.
