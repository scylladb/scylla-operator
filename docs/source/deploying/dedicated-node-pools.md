...
Key flags:
- `--image-type='UBUNTU_CONTAINERD'` — GKE Container OS is not supported (see [Prerequisites](../installation/prerequisites.md)).
- `--local-nvme-ssd-block` — attaches local NVMe SSDs for ScyllaDB data.
- `--system-config-from-file` — enables the kubelet static CPU manager policy (see [CPU pinning](cpu-pinning.md)).
...
- `amiFamily: AmazonLinux2023` — Bottlerocket is not supported (see [Prerequisites](../installation/prerequisites.md)).
- `cpuManagerPolicy: static` — enables CPU pinning (see [CPU pinning](cpu-pinning.md)).
...
- [Node configuration](node-configuration.md) — configuring disk setup, sysctls, and performance tuning.
- [CPU pinning](cpu-pinning.md) — configuring CPU exclusivity for ScyllaDB containers.
- [Deploying a single-DC cluster](single-dc-cluster.md) — creating a ScyllaCluster with placement.
- [Deploying a multi-DC cluster](multi-dc-cluster.md) — creating a ScyllaDBCluster with placement.
- [Prerequisites](../installation/prerequisites.md) — supported platforms and node requirements.
