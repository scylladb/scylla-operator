...
- Dedicated node pool for ScyllaDB (see [Dedicated node pools](dedicated-node-pools.md))
- `xfsprogs` available on the host OS (see [Prerequisites](../installation/prerequisites.md))
...
Container-level tuning runs every time a ScyllaDB container starts and **blocks ScyllaDB startup** until it completes (via the [ignition mechanism](../architecture/ignition.md)). This ensures ScyllaDB always starts with the correct tuning.
...
For details on how tuning interacts with CPU pinning, see [CPU pinning](cpu-pinning.md) and [Tuning architecture](../architecture/tuning.md).
...
- [Dedicated node pools](dedicated-node-pools.md) — setting up isolated nodes for ScyllaDB.
- [CPU pinning](cpu-pinning.md) — configuring CPU exclusivity for ScyllaDB containers.
- [Tuning architecture](../architecture/tuning.md) — how node-level and container-level tuning work.
- [Storage architecture](../architecture/storage.md) — how NodeConfig, Local CSI Driver, and PVs work together.
