...
:::{note}
Configuring the CPU manager policy is required only on nodes where ScyllaDB pods will run. It does not need to be set cluster-wide. See [CPU pinning](../deploying/cpu-pinning.md) for a full walkthrough.
:::
...
For multi-datacenter deployments across Kubernetes clusters, pod-to-pod communication must be routed between clusters. See [Multi-DC infrastructure](multi-dc-infrastructure.md) for platform-specific firewall configuration.
...
- [GitOps installation](gitops.md) — step-by-step installation using manifests.
- [Helm installation](helm.md) — step-by-step installation using Helm charts.
- [Dedicated node pools](../deploying/dedicated-node-pools.md) — configuring ScyllaDB to run on dedicated nodes.
- [Node configuration](../deploying/node-configuration.md) — setting up NodeConfig for disk and performance tuning.
