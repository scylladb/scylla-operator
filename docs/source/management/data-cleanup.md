...
2. **Data resurrection**: If this stale data is not removed, it can lead to data resurrection later on.
...
:::{note}
Due to {{productName}} relying strictly on the token ring changes, there are some limitations which we describe in the
[Known limitations](#known-limitations) section.
:::

## Inspecting cleanup jobs

{{productName}} creates a Job per node that requires cleanup. If any job is still running, the `ScyllaCluster`
status contains the `JobControllerProgressing` condition set to `True`. When a job completes successfully, it's removed
from the cluster by {{productName}}. The condition's message contains name of the running job(s).
...
When a node is decommissioned, tokens are redistributed to other remaining nodes. These nodes do not require cleanup since
they are not losing any tokens. However, {{productName}} still triggers cleanup for these nodes due to the token ring change.
This is a safe operation, but it may lead to unnecessary spike in I/O load.

### Missed cleanup on RF changes

When the replication factor (RF) of a keyspace is decreased, the token ring remains unchanged. Thus, {{productName}} does
...
manually in such cases (run on an arbitrary node):
