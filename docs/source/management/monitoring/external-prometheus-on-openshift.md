...
permissions to create `ServiceAccount`s and `ClusterRoleBinding`s.
...
The `Secret` will be populated with the token automatically by Kubernetes and should be available under the `token` key of this `Secret`.
