...
If the replacement gets stuck — for example, the new pod enters `CrashLoopBackOff` or streaming cannot complete — see [Recovering from a failed replace](../troubleshooting/recovering-from-failed-replace.md) for a step-by-step fallback procedure.
...
- [Scaling](scaling.md) — adding or removing nodes without replacement
- [StatefulSets and racks](../architecture/statefulsets-and-racks.md) — pod identity and ordinal management
- [Recovering from a failed replace](../troubleshooting/recovering-from-failed-replace.md) — fallback when replacement is stuck
- [Rolling restart](rolling-restart.md) — restarting nodes without replacement
