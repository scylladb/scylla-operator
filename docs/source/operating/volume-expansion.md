...
- [Scaling a ScyllaDB cluster](scaling.md) — changing the number of nodes instead of the volume size
- [StatefulSets and racks](../architecture/statefulsets-and-racks.md) — why each rack maps to a StatefulSet and what constraints that implies
- [Replacing nodes](replacing-nodes.md) — replacing a node with a fresh PVC rather than expanding the existing one
