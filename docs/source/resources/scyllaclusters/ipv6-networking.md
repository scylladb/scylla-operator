...
When you configure IPv6 networking via `network.ipFamilies`, ScyllaDB Operator automatically configures:
...
Here's what happens internally when IPv6 is configured (via `network.ipFamilies: ["IPv6"]`):
...
**Important**: These configurations are applied automatically by the operator - you don't need to set them manually.

**Note**: The broadcast addresses (`--broadcast-address` and `--broadcast-rpc-address`) are determined by your `exposeOptions.broadcastOptions` configuration, not by directly setting ScyllaDB arguments. The operator automatically ensures they use the correct IP family based on your network configuration.

## Configuration Options

### IPv6 Networking Configuration

Configure IPv6 or dual-stack networking using the `network` field:
...
**IP Family Selection**:
- The **first** IP family in `ipFamilies` determines which protocol ScyllaDB uses internally
- `ipFamilies: ["IPv6"]` → ScyllaDB uses IPv6
- `ipFamilies: ["IPv4", "IPv6"]` → ScyllaDB uses IPv4, services support both
- `ipFamilies: ["IPv6", "IPv4"]` → ScyllaDB uses IPv6, services support both
...
The first IP family in `ipFamilies` automatically configures both node and client broadcast addresses to use the same IP family, ensuring consistency across your ScyllaDB cluster.
...
To run ScyllaDB exclusively on IPv6, configure the network settings. The operator handles all the rest:
...
- **Single IP family clusters**: Never needed - the network configuration is sufficient

**Client Connectivity in Dual-Stack**

Even with dual-stack services, ScyllaDB itself only communicates on one IP family:
- If `ipFamilies: [IPv4, ...]` → ScyllaDB nodes and clients communicate via IPv4
- If `ipFamilies: [IPv6, ...]` → ScyllaDB nodes and clients communicate via IPv6
- Kubernetes services can be reachable via both protocols, but the actual database connection uses the first IP family
...
- Actual ScyllaDB connections still use IPv4 (because IPv4 is first in list)
...
1. **Network IP Families**: Uses the first IP family from `network.ipFamilies` 
2. **Fallback**: Defaults to IPv4 if no configuration is found (backward compatibility)
...
- `network.ipFamilies: ["IPv6", "IPv4"]` → Uses IPv6 for ScyllaDB (first in list)
...
- ScyllaDB nodes communicate using IPv4 (the first IP family in the list)
- Kubernetes services are accessible via both IPv4 and IPv6 addresses
- Clients can discover and connect to services using either protocol
- The service layer translates between client IP family and ScyllaDB's IPv4

This is useful when you have clients on IPv6-only networks that need to reach an IPv4 ScyllaDB cluster.
...
spec:
  network:
    ipFamilyPolicy: SingleStack
    ipFamilies:
      - IPv6  # Consistent across all DCs
  datacenter:
    name: dc1
    # ...
