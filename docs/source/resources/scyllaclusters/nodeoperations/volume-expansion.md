...
1. Since we are going to delete the ScyllaCluster object, make sure to have a copy of the existing one. You will need it later.

2. Orphan delete ScyllaCluster
    ```bash
...
3. Orphan delete the underlying ScyllaDBDatacenter. It has the same name as ScyllaCluster.
   ...
   ```
4. Orphan delete the underlying StatefulSets
    ```bash
...
7. Update saved ScyllaCluster definition with the new storage size `spec.datacenter.racks[*].storage.capacity` field, and apply it:
   ...
   The StatefulSets will be recreated by the Scylla Operator with the new storage size.
