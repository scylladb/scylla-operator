...
Examples and additional documentation pages will mention new API
which will bring these improvements to more adventurous users.
...
- `cpuSet` is confusing, as it doesn't change anything related to cpuset, it only controls stricter validation of allocated resources.
  ...
  This field will be removed and validation will be relaxed.
- We initially thought that `hostNetworking` helps with performance, but later we discovered we were wrong.
  ...
  annotation `scylla-operator.scylladb.com/enable-host-networking` to `true` on `v1alpha1.ScyllaDBDatacenter`.
...
To improve it, the new field `toplogyLabelSelector` added on the datacenter and rack level,
...
To prevent misuse of this simplification keys of the provided label selector will be validated against the `topology.kubernetes.io/*` rule,
and usage of both `nodeAffinity` and `topologyLabelSelector` will be forbidden by the webhook.
Users wanting to construct more complex placement rules will have to do it using `placement` spec.
...
while preserving the existing flexibility of having non-uniform racks by overriding dedicated specifications.
Two characteristics make racks uniform: the number of nodes and allocated resources.

#### Resources

Resources for both used components (ScyllaDB and Agent) will be available both on the datacenter level and on the rack level.
Rack level settings override ones set on the datacenter level.

#### Node count

A new field `nodes` will be added on the datacenter level. Scylla Operator will balance this number between all defined racks.
Having a single place where the desired number of nodes is specified, enables us to have support for scale subresource,
which brings users handy scaling operations using `kubectl scale` and paves a path towards available Kubernetes autoscalers.
The downside of it is that these won't be able to add more racks, but provides a nice way of scaling for static topology clusters.

Datacenter level `nodes` cannot be used together with `nodes` specified per rack. Webhook will prevent it.

The balancing algorithm is iterative, adding/removing one node at a time in each iteration to ensure that racks stay as balanced as possible.

```go
func balanceNodesAcrossRacks(racks []*appsv1.StatefulSet, totalNodes int) []*appsv1.StatefulSet {
    // Calculate the total number of existing nodes
    existingNodes := 0
    for _, rack := range racks {
       existingNodes += rack.Replicas
    }

    if existingNodes == totalNodes {
       // The desired number of nodes reached, do nothing.
       return racks
    }
    
    if existingNodes < totalNodes {
       // Add a node to the least populated rack
       minRackIndex := findSmallestStatefulSet(racks)
       racks[minRackIndex].Replicas++
       return balanceNodesAcrossRacks(racks, totalNodes)
    }
    
    // Remove a node from biggest rack.
    maxRackIndex := findBiggestStatefulSet(racks)
    racks[maxRackIndex].Replicas--
    return balanceNodesAcrossRacks(racks, totalNodes)
}
```

New Condition will be added to the `status` of `v1alpha1.ScyllaDBDatacenter`, providing a simple way of noticing rack imbalance.
Its type is going to be `UnbalancedRacks` and the value of it will be reconciled accordingly to the state of rack imbalance.
...
type ScyllaDBDatacenterSpec struct {
    ...
    // datacenterName is the name of the scylla datacenter. Used as datacenter name in GossipingPropertyFileSnitch.
    ...
    // nodes specify desired total number of nodes in this datacenter.
    // This number will be balanced out across all specified racks.
    // Cannot be used together with `nodes` specified within any rack spec.
    // +optional
...
// ScyllaDBOverrides allows to override a subset of ScyllaDB settings.
type ScyllaDBOverrides struct {
...
// ScyllaDBManagerAgentOverrides allows to override a subset of ScyllaManagerAgent settings.
type ScyllaDBManagerAgentOverrides struct {
...
type RackSpec struct {
    ...
    // nodes specifies desired number of nodes in this rack.
    // +optional
    Nodes *int32 `json:"nodes"`
    ...
    // scyllaDB provides overrides for properties of ScyllaDB for this rack.
    // They override the settings set on Datacenter level.
    ScyllaDB *ScyllaDBOverrides `json:"scyllaDB"`

    // scyllaDBManagerAgent provides overrides for properties of ScyllaDB Manager Agent for this rack.
    // They override the settings set on Datacenter level.
    ScyllaDBManagerAgent *ScyllaDBManagerAgentOverrides `json:"scyllaDBManagerAgent"`
}
...
type ScyllaDB struct {
    ...
    // scyllaArgs will be appended to the ScyllaDB binary during startup.
    ...
    // resources requirements for the ScyllaDB container in entire datacenter.
    // These may be overwritten for particular rack in RackSpec.
    // +optional
    Resources *corev1.ResourceRequirements `json:"resources,omitempty"`

    // storage requirements for the containers in entire datacenter.
    // These may be overwritten for particular rack in RackSpec.
    // +optional
    Storage *Storage `json:"storage,omitempty"`
}
...
type ScyllaDBManagerAgent struct {
    ...
    // resources are requirements for the ScyllaDB Manager Agent container in entire datacenter.
    // These may be overwritten for particular rack in RackSpec.
    // +optional
    Resources *corev1.ResourceRequirements `json:"resources,omitempty"`
}
...
Existing `v1.ScyllaCluster` is going to be converted to `v1alpha.ScyllaDBDatacenter` and controllers are going to work on both of these resources.
When rolled back, changes to `v1alpha1.ScyllaDBDatacenters` are not going to be reconciled, users have to continue using `v1.ScyllaCluster`.

## Implementation History

26.06.204 - Initial version

## Drawbacks

Such change is not something we couldn't live with, but it helps to reduce the ambiguity of naming mistakes,
as well as reduce the technical dept.
