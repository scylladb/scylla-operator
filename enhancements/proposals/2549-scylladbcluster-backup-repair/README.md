...
- Introduce support for scheduling of backup tasks for ScyllaDBClusters with ScyllaDB Manager.
- Introduce support for scheduling of repair tasks for ScyllaDBClusters with ScyllaDB Manager.
...
- Deprecate `scyllaclusters.scylla.scylladb.com/v1.spec.backups` and `scyllaclusters.scylla.scylladb.com/v1.spec.repairs`.
- Introduce support for removing ScyllaDBClusters and ScyllaCluster from ScyllaDB Manager state on objects' deletion [#1930][2].
...
Simultaneously, `scyllaclusters.scylla.scylladb.com/v1.spec.backups` and `scyllaclusters.scylla.scylladb.com/v1.spec.repairs` will be deprecated and users advised to use the new API.
...
#### Multi-datacenter ScyllaDB cluster registration with ScyllaDB Manager
...
#### ScyllaDB Manager backup and repair task scheduling for multi-datacenter ScyllaDB clusters

As a user, I want to schedule backup and repair tasks for multi-datacenter ScyllaDB clusters with ScyllaDB Manager.
To do that, I create ScyllaDBManagerTask (scylla.scylladb.com/v1alpha1) objects referencing a ScyllaDBCluster.

#### ScyllaDB Manager backup and repair task scheduling for single-datacenter ScyllaDB clusters

As a user, I want to maintain the possibility of scheduling backup and repair tasks for single-datacenter ScyllaDB clusters with ScyllaDB Manager.
To do that, I either:
...
- Configure the legacy ScyllaCluster (scylla.scylladb.com/v1) object with deprecated `scyllaclusters.scylla.scylladb.com/v1.spec.backups` and `scyllaclusters.scylla.scylladb.com/v1.spec.repairs`.
...
ScyllaDBManagerTask objects, created by the translation controller, adhere to the following naming convention to avoid name collisions:
- Tasks created from `scyllaclusters.scylla.scylladb.com/v1.spec.backups`: SHA256(`scyllaclusters.scylla.scylladb.com/v1.name`, "backup", `scyllaclusters.scylla.scylladb.com/v1.spec.backups[].name`).
- Tasks created from `scyllaclusters.scylla.scylladb.com/v1.spec.repairs`: SHA256(`<scyllaclusters.scylla.scylladb.com/v1.name>`, "repair", `scyllaclusters.scylla.scylladb.com/v1.spec.repairs[].name`).
SHA256 hash is to reduce the length of the produced name, as both ScyllaCluster's and corresponding task's name could have max allowed name length. Hexadecimal representation of SHA256 is 64 characters.
...
- Schedule (shared by backup and repair specifications):
  ...
  - The type of `StartDate` field is changed to `*metav1.Time`. With that we no longer support the special `now[+duration]` syntax in the new API.
    ...
    To maintain backwards compatibility with `now[+duration]` syntax, the translating controller will calculate the value based on `scyllaclusters.scylla.scylladb.com/v1.status`.
