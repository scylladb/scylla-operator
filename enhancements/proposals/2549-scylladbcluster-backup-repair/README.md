...
GlobalScyllaDBManager controller will create a ScyllaDBManagerClusterRegistration in the reserved `scylla-manager` namespace for all ScyllaDBCluster and ScyllaDBDatacenter objects with `"scylla-operator.scylladb.com/register-with-global-scylladb-manager": "true"` label across all namespaces.
No other clusters in ScyllaDB Manager's state will be subject to reconciliation.

The ScyllaDBManagerClusterRegistration created by GlobalScyllaDBManager controller will have an internal annotation set up `"internal.scylla-operator.scylladb.com/global-scylladb-manager": "true"`.

Because the reference to ScyllaDBCluster or ScyllaDBDatacenter defined in ScyllaDBManagerClusterRegistration's specification is local, ScyllaDBManagerClusterRegistration objects created by the GlobalScyllaDBManager controller will have an internal annotation set up `internal.scylla-operator.scylladb.com/scylladbcluster-ref-namespace-override`, with its value set to the namespace of the target ScyllaDBDatacenter or ScyllaDBCluster, allowing for overriding the implied local namespace.
...
concatenate(
  truncate(
    concatenate(SCYLLADB_CLUSTER_NAMESPACE, '-', toLower(SCYLLADB_CLUSTER_KIND), '-', SCYLLADB_CLUSTER_NAME), 
    min(len(concatenate(SCYLLADB_CLUSTER_NAMESPACE, '-', SCYLLADB_CLUSTER_KIND, '-', SCYLLADB_CLUSTER_NAME)), DNS1123SubdomainMaxLength - 5)
  ),
  truncate(FNV-1a(SCYLLADB_CLUSTER_NAMESPACE, SCYLLADB_CLUSTER_KIND, SCYLLADB_CLUSTER_NAME), 5)
)
```
...
For the time being, only ScyllaDBManagerClusterRegistration objects with the internal annotation `"internal.scylla-operator.scylladb.com/global-scylladb-manager": "true"` will be reconciled.
...
ScyllaDBManagerClusterRegistration controller will set the name of the clusters registered in the global ScyllaDB Manager's state to `scylladbmanagerclusterregistration.scylla.scylladb.com/v1alpha1.metadata.name`.

To ensure continuity with the existing state of global ScyllaDB Manager instance, we will enable overriding the name with an internal annotation `"internal.scylla-operator.scylladb.com/scylladb-manager-cluster-registration-name-override"`.
...
The translation controller will set the internal annotation `"internal.scylla-operator.scylladb.com/scylladb-manager-cluster-registration-name-override": "<scyllaclusters.scylla.scylladb.com/v1.metadata.namespace>/<scyllaclusters.scylla.scylladb.com/v1.metadata.name>"` on all ScyllaDBDatacenters created as a result of translation.
...
ScyllaDBManagerClusterRegistration controller, running as part of the operator, will set a `scylla-operator.scylladb.com/scylladbmanagerclusterregistration-protection` finalizer on the reconciled object so that the clusters are deleted from ScyllaDB Manager state on deletion [#1930][2].
...
All fields will be validated. Only the objects with `"internal.scylla-operator.scylladb.com/global-scylladb-manager": "true"` internal annotation will be admitted.
...
To ensure continuity with the existing state of global ScyllaDB Manager instance, we will enable overriding the name with an internal annotation `"internal.scylla-operator.scylladb.com/scylladb-manager-task-name-override"`.
...
The new ScyllaDBManagerTask controller will set a `scylla-operator.scylladb.com/scylladbmanagertask-protection` finalizer on ScyllaDBManagerTask objects so that the tasks are deleted from ScyllaDB Manager state on object deletion.
...
Simultaneously, the not removed manager-controller will keep reconciling ScyllaClusters as is, leading to conflicts.
Since we will retain the naming convention for ScyllaClusters registered with ScyllaDB Manager, as well as their scheduled tasks, the conflict should not lead to creating multiple cluster or task instances in ScyllaDB Manager state.
Minor changes in cluster or task specification, used for managed hash calculation in either controller, might cause the controllers to end up in an update hot loop.

We can consider implementing a best-effort approach to preventing this scenario by refusing to start Scylla Operator if manager-controller deployment is discovered in `scylla-manager`.
...
Users can still opt out of using ScyllaDB Manager with Scylla Operator by not deploying the global ScyllaDB Manager server instance. We can consider checking for its presence in GlobalScyllaDBManager controller to avoid congesting the logs with misleading errors.
...
Objects will however be created in the reserved `scylla-manager` namespace so cleaning them up should not introduce noticeable difficulties when uninstalling the global ScyllaDBManager instance.
