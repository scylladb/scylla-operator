...
The approach is inspired by kube-controller-manager's, where since Kubernetes 1.36 the DaemonSet, StatefulSet,
ReplicaSet and Job controllers check a `ConsistencyStore` of their own writes against `LastStoreSyncResourceVersion()`
before a sync
([blog post](https://kubernetes.io/blog/2026/04/28/kubernetes-v1-36-staleness-mitigation-for-controllers/)). One
difference matters for section 5: upstream keys the writes per owning object, ours per controller (3.2).
...
configured may change; the design is still under discussion in
[controller-runtime#3473](https://github.com/kubernetes-sigs/controller-runtime/pull/3473). The flag is set in one
place, the operator's manager
...
96 lines). Two small things our controllers need on top of what controller-runtime provides: `Requeue`, which
accumulates the shortest of the delays a sync used to pass to `queue.AddAfter`; and `Trigger` over a `source.Channel`,
with `PeriodicTrigger` as a manager runnable on top, for anything outside the watches that needs to wake a controller,
a timer or a test. The observer abstraction they replace was 159 lines.

The third helper, `EnqueueSingleton`, is for observers that reconcile a singleton rather than an object: a fixed
`reconcile.Request` returned from `EnqueueRequestsFromMapFunc`. controller-runtime's own idiom for a singleton is a
typed controller whose request type is an empty struct, which the workqueue deduplicates
([`examples/typed`](https://github.com/kubernetes-sigs/controller-runtime/blob/6ab2188a1fb14a8c4837b418bc0a20049682cde4/examples/typed/main.go#L50-L68)).
The PoC kept `reconcile.Request` so that the eight singleton controllers share the reconciler shape and helpers of the
rest; moving them to a typed request is recommended for milestone 2.
...
as clusters appear, and the multi-DC e2e jobs passed. It was the least comfortable part of the work: a watch attached to
a running controller cannot be detached, because `controller.Watch` starts the source under the controller's context and
the source's start goroutine waits for the cache and its handler to sync
([`kind.go:78`](https://github.com/kubernetes-sigs/controller-runtime/blob/v0.25.0/pkg/internal/source/kind.go#L78-L131)),
so a cluster replaced while that wait is in progress keeps its stopped cache alive until the controller stops
(`cache.RemoveInformer` stops one informer of a running cache and does not help here); the PoC's cluster set reports a
cluster ready once its cache is started rather than synced, so the first reads against it fail with `ErrCacheNotStarted`
or block until the sync ([`set.go:182`](https://github.com/czeslavo/scylla-operator/blob/f334d7813/pkg/remotecluster/set.go#L182-L211));
the upstream answer, `sigs.k8s.io/multicluster-runtime`, is itself experimental; and none of it has envtest coverage.
The branch keeps that code as evidence that the port is possible should the product decision change.
...
**One informer per kind per namespace.** This is the real constraint. A cache holds one informer per kind,
representation and namespace entry, so two different selectors on the same kind in the same namespace, or both across
all namespaces, cannot coexist in one cache. The choice is then to widen the selector and filter client-side, or to run
a second cache. When the two selectors partition by namespace, `ByObject.Namespaces` expresses them directly: each entry
is its own informer with its own selectors, and controller-runtime appends `metadata.namespace!=<ns>` for every named
namespace to the `AllNamespaces` entry
([`cache.go:534`](https://github.com/kubernetes-sigs/controller-runtime/blob/v0.25.0/pkg/cache/cache.go#L534-L541)),
so a `Get` or `List` is served by exactly one of them. The PoC hit the constraint once, and widened once where it did
not need to:
...
- Node setup had two Pod factories with disjoint selectors, ScyllaDB Pods on the node across all namespaces
  ...
  name ([`:203`](https://github.com/scylladb/scylla-operator/blob/5d041a933/pkg/cmd/operator/nodesetupdaemon.go#L203)). The PoC folded them into one informer on `spec.nodeName` alone
  ...
  predicate ([`controller.go:150`](https://github.com/czeslavo/scylla-operator/blob/f334d7813/pkg/controller/nodetune/controller.go#L150)), so the daemon caches every
  Pod on its node. The per-namespace form is closer to master: `AllNamespaces` on `spec.nodeName` plus the ScyllaDB
  labels, and the daemon's own namespace on `spec.nodeName` alone, which covers the daemon's Pod and any ScyllaDB Pod
  that happens to live there, at the cost of caching that node's perftune Job Pods. Filtering that namespace by
  `metadata.name` instead would hide ScyllaDB Pods in it, because the `AllNamespaces` informer excludes the namespace.
  The PoC should adopt the per-namespace form.
- The operator binary had a dedicated ScyllaOperatorConfig factory filtered to `metadata.name=cluster`
  ([`operator.go:320`](https://github.com/scylladb/scylla-operator/blob/5d041a933/pkg/cmd/operator/operator.go#L320)) next to the unfiltered shared one. The PoC kept the
  unfiltered informer plus a name predicate ([`controller.go:75`](https://github.com/czeslavo/scylla-operator/blob/f334d7813/pkg/controller/scyllaoperatorconfig/controller.go#L75)).
  That was a choice, not the constraint: every consumer reads the singleton, so a
  `ByObject[&ScyllaOperatorConfig{}].Field` on its name would serve all of them from one filtered informer.
...
| Granularity, and what it couples | per controller, all kinds, once per sync: a pending write for one ScyllaDBDatacenter delays the sync of every other one, on every worker. A choice of the PoC, not of the mechanism: kube-controller-manager keys the same store per owning object (3.1) | per kind, process-wide, per list call: a reconcile that lists Pods waits for Pod writes by any controller; an SDC reconcile makes about a dozen such lists. For completed writes the cost is informer lag, for in-flight writes it is that request's latency |
...
| Control over what is watched | full: one factory per need, any selector | declared per controller via `For`/`Owns`/`Watches` and per cache via options, with an equivalent for every tweak we use; one informer per kind per namespace, so two selectors on one kind must partition by namespace or need a second cache (4.6) |
