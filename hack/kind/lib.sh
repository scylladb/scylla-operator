...
# The image is pushed with tag $3 (defaults to "latest").
# Usage: build-and-push-operator-image <root_dir> <image_var_name> [<tag>]
function build-and-push-operator-image {
  ...
  local tag_ref="${KIND_REGISTRY_HOST}:${KIND_REGISTRY_PORT}/scylladb/scylla-operator:${tag}"
