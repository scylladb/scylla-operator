...
| **`nodetool`**                | `kubectl exec` + `nodetool`                             | Run `nodetool` commands by executing into the ScyllaDB container: `kubectl exec -it <pod> -c scylla -- nodetool status`. Read-only commands are safe; state-changing commands are (mostly) disallowed; see [nodetool alternatives](https://operator.docs.scylladb.com/master/reference/nodetool-alternatives.md).                            |
...
For detailed troubleshooting procedures, see the [troubleshooting guide](https://operator.docs.scylladb.com/master/troubleshoot/index.md).
