...
You need an OpenShift Container Platform cluster in a [supported version](https://operator.docs.scylladb.com/master/reference/releases.md) with:

- **A dedicated machine pool for ScyllaDB** with local NVMe storage and sufficient CPU and memory.
  See [Set up dedicated node pools](https://operator.docs.scylladb.com/master/deploy-scylladb/before-you-deploy/set-up-dedicated-node-pools.md) for labeling and taint requirements.
...
- [Install ScyllaDB Operator on OpenShift](https://operator.docs.scylladb.com/master/install-operator/install-on-openshift.md) — install the operator via OLM.
