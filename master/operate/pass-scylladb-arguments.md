...
When a rolling restart is not possible — for example, during a stuck rollout or with a degraded cluster — use the ScyllaDB REST API to change settings on running pods without a restart. See [Change the log level](https://operator.docs.scylladb.com/master/troubleshoot/change-log-level.md).

## Related pages

- [Perform a rolling restart](https://operator.docs.scylladb.com/master/operate/perform-rolling-restart.md) — how the Operator performs rolling restarts when the pod spec changes
- [Changing the log level](https://operator.docs.scylladb.com/master/troubleshoot/change-log-level.md) — adjusting ScyllaDB runtime settings without a restart
