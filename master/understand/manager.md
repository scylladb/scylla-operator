...
- Waits for [ignition](https://operator.docs.scylladb.com/master/understand/ignition.md) and for the ScyllaDB REST API (port 10000) to become available before starting — this covers the gap between ignition completing and ScyllaDB finishing IO tuning on first boot.
...
- **Restore** is not yet available through the Operator’s declarative API. To restore from a Manager backup, you must exec into the Manager pod and use `sctool` directly. See [Back up and restore](https://operator.docs.scylladb.com/master/operate/back-up-and-restore.md).
