...
|   3 | `scylladb-bootstrap-barrier` | ScyllaDB | Blocks until all nodes in the cluster report each other as UP, preventing a new node from bootstrapping into an unhealthy cluster. See [Bootstrap synchronisation](https://operator.docs.scylladb.com/master/understand/bootstrap-sync.md). | Only when the `BootstrapSynchronisation` feature gate is enabled and the ScyllaDB version is ≥ 2025.2 |

### Long-running containers

| Name                        | Image                  | Purpose                                                                                                                                                                                                                 | Conditional                            |
|-----------------------------|------------------------|-------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------|----------------------------------------|
| `scylla`                    | ScyllaDB               | The main container. Waits for the ignition signal, then execs the sidecar binary which configures and starts ScyllaDB.                                                                                                  | Always present                         |
| `scylladb-api-status-probe` | Operator               | Translates ScyllaDB’s native HTTP API status into Kubernetes health-check endpoints (`/healthz`, `/readyz`) on port 8080.                                                                                               | Always present                         |
| `scylladb-ignition`         | Operator               | Evaluates startup prerequisites (tuning done, IPs assigned, container running) and creates the ignition signal file when all are met. See [Ignition](https://operator.docs.scylladb.com/master/understand/ignition.md). | Always present                         |
| `scylla-manager-agent`      | Manager Agent          | Runs the ScyllaDB Manager Agent for backup, repair, and health-check operations. Waits for the ignition signal and for the ScyllaDB REST API to become available before starting.                                       | Always present                         |
| `scylladb-node-exporter`    | ScyllaDB Node Exporter | Exposes node-level OS metrics (CPU, disk, network) for Prometheus to scrape.                                                                                                                                            | Only when ScyllaDB version is ≥ 2026.3 |
...
| Data PVC                            | `/var/lib/scylla`                                                                                       | Persistent storage for ScyllaDB data files. Mounted read-write in the main container, read-only in the bootstrap barrier and scylladb-node-exporter sidecars. |
...
This mechanism ensures that ScyllaDB never starts before node-level tuning is complete and network addresses are assigned. See [Ignition](https://operator.docs.scylladb.com/master/understand/ignition.md) for the full list of prerequisites.
...
- Updates the Service annotations with the current host ID and token ring hash. These annotations are consumed by the datacenter controller to detect token ring changes (for [automatic data cleanup](https://operator.docs.scylladb.com/master/understand/automatic-data-cleanup.md)) and to track node identity.
...
- [Ignition](https://operator.docs.scylladb.com/master/understand/ignition.md) — detailed startup prerequisites and the ignition controller.
- [Bootstrap synchronisation](https://operator.docs.scylladb.com/master/understand/bootstrap-sync.md) — the barrier init container.
- [Tuning](https://operator.docs.scylladb.com/master/understand/tuning.md) — how node tuning feeds into ignition readiness.
- [Networking](https://operator.docs.scylladb.com/master/understand/networking.md) — broadcast address types and Service model.
- [Security](https://operator.docs.scylladb.com/master/understand/security.md) — TLS certificates mounted into pod containers and Manager Agent authentication.
- [StatefulSets and racks](https://operator.docs.scylladb.com/master/understand/statefulsets-and-racks.md) — pod identity, scaling, and decommission flow.
