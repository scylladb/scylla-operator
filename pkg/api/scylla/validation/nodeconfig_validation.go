...
func validateSysctls(sysctls []corev1.Sysctl, fldPath *field.Path) field.ErrorList {
	...
	for i, s := range sysctls {
		if len(s.Name) == 0 {
			...
			allErrs = append(allErrs, field.Invalid(fldPath.Index(i).Child("name"), s.Name, fmt.Sprintf("must have at most %d characters and match regex %s", corevalidation.SysctlMaxLength, corevalidation.SysctlContainSlashRegexp)))
