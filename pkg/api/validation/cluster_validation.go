...
func ValidateScyllaClusterSpec(spec *scyllav1.ScyllaClusterSpec, fldPath *field.Path) field.ErrorList {
	...
	for _, rack := range spec.Datacenter.Racks {
		allErrs = append(allErrs, ValidateScyllaClusterRackSpec(rack, rackNames, spec.CpuSet, fldPath.Child("datacenter", "racks").Key(rack.Name))...)
	}

	managerTaskNames := strset.New()
	for _, r := range spec.Repairs {
		if managerTaskNames.Has(r.Name) {
			allErrs = append(allErrs, field.Duplicate(fldPath.Child("repairs").Key(r.Name), r.Name))
		}
		managerTaskNames.Add(r.Name)

		_, err := strconv.ParseFloat(r.Intensity, 64)
		if err != nil {
			allErrs = append(allErrs, field.Invalid(fldPath.Child("repairs").Key(r.Name), r.Intensity, "invalid intensity, it must be a float value"))
		}
	}

	for _, b := range spec.Backups {
		if managerTaskNames.Has(b.Name) {
			allErrs = append(allErrs, field.Duplicate(fldPath.Child("backups").Key(b.Name), b.Name))
		}
...
func ValidateScyllaClusterUpdate(new, old *scyllav1.ScyllaCluster) field.ErrorList {
	...
	return ValidateScyllaClusterSpecUpdate(&new.Spec, &old.Spec, field.NewPath("spec"))
}
...
func ValidateScyllaClusterSpecUpdate(newSpec, oldSpec *scyllav1.ScyllaClusterSpec, fldPath *field.Path) field.ErrorList {
	...
	for _, newRack := range newSpec.Datacenter.Racks {
		...
		if !reflect.DeepEqual(oldRack.Placement, newRack.Placement) {
			allErrs = append(allErrs, field.Forbidden(fldPath.Child("datacenter", "racks").Key(oldRack.Name), "changes in placement are currently not supported"))
		}

		// Check that storage is the same as before
		if !reflect.DeepEqual(oldRack.Storage, newRack.Storage) {
			allErrs = append(allErrs, field.Forbidden(fldPath.Child("datacenter", "racks").Key(oldRack.Name), "changes in storage are currently not supported"))
		}

		// Check that resources are the same as before
		if !reflect.DeepEqual(oldRack.Resources, newRack.Resources) {
			allErrs = append(allErrs, field.Forbidden(fldPath.Child("datacenter", "racks").Key(oldRack.Name), "changes in resources are currently not supported"))
		}
	}
