...
import (
	...
	"github.com/scylladb/scylla-operator/pkg/cmd/operator/serveprobes"
...
type IgnitionOptions struct {
	...
	serveprobes.ProbeServerOptions
...
func NewIgnitionOptions(streams genericclioptions.IOStreams) *IgnitionOptions {
	...
	return &IgnitionOptions{
		ProbeServerOptions: *serveprobes.NewProbeServerOptions(streams, naming.ScyllaDBIgnitionProbePort, mux),
		ClientConfig:       genericclioptions.NewClientConfig("scylla-operator-api-status-probe"),
		mux:                mux,
		ignited:            atomic.Bool{},
	}
}
...
func (o *IgnitionOptions) AddFlags(cmd *cobra.Command) {
	...
	o.ProbeServerOptions.AddFlags(cmd)
...
func (o *IgnitionOptions) Validate(args []string) error {
	...
	errs = append(errs, o.ProbeServerOptions.Validate(args))
...
func (o *IgnitionOptions) Complete(args []string) error {
	...
	err = o.ProbeServerOptions.Complete(args)
	if err != nil {
		return err
	}
...
func (o *IgnitionOptions) RunCtx(ctx context.Context, originalStreams genericclioptions.IOStreams, cmd *cobra.Command) (returnErr error) {
	startTime := time.Now()
	...
	o.mux.HandleFunc(naming.ReadinessProbePath, healthzFunc)
	o.mux.HandleFunc(naming.LivenessProbePath, healthzFunc)

	wg.Add(1)
	go func() {
		defer wg.Done()

		err := o.ProbeServerOptions.RunCtx(ctx, originalStreams, cmd)
		if err != nil {
	...
	klog.InfoS("Waiting for Service availability and IP address",
		"Service", naming.ManualRef(o.Namespace, o.ServiceName),
	)
	...
	if !ok {
		return fmt.Errorf("internall error: unexpected object type %T", cmEvent.Object)
	}
