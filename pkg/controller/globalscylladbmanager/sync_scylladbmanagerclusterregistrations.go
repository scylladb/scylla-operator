...
func makeScyllaDBManagerClusterRegistrationsForObjects[S ~[]E, E metav1.Object](
	...
	requiredScyllaDBManagerClusterRegistrations := map[string][]*scyllav1alpha1.ScyllaDBManagerClusterRegistration{}
	for _, x := range xs {
		required, err := makeScyllaDBManagerClusterRegistrationForObject(x)
		if err != nil {
			errs = append(errs, fmt.Errorf("can't make ScyllaDBManagerClusterRegistration for object %q: %w", naming.ObjRef(x), err))
			continue
		}

		requiredScyllaDBManagerClusterRegistrations[x.GetNamespace()] = append(requiredScyllaDBManagerClusterRegistrations[x.GetNamespace()], required)
	}
