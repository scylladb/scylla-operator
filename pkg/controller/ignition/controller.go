...
// isIdentityServiceReady determines whether the other controllers have finished
// setting up the infra and updated the Service.
// TODO: This is not bound to the lifecycle of the ScyllaDB Pod and should be evaluated in the controller.
func (c *Controller) isIdentityServiceReady(ctx context.Context) (bool, error) {

	return true, nil
}

func (c *Controller) hasPodContainerIDAndIP(ctx context.Context) (bool, error) {
...
func (c *Controller) Sync(ctx context.Context) error {
	...
	// TODO: This isn't bound to the lifecycle of the ScyllaDB Pod and should be evaluated in the controller.
	...
	if len(containerID) == 0 {
		klog.V(2).InfoS("ContainerID is not yet set", "Pod", klog.KObj(pod), "UID", pod.UID)
		ignited = false
	} else {
		klog.V(2).InfoS("Pod has ContainerID set", "Pod", klog.KObj(pod), "UID", pod.UID, "ContainerID", containerID)
	}
