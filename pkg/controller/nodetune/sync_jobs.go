...
func (ncdc *Controller) makeResourceLimitJobsForContainers(ctx context.Context, podSpec *corev1.PodSpec, scyllaPods []*corev1.Pod, scyllaContainerIDs []string) (*batchv1.Job, error) {
	scyllaProcessHostPIDs := make([]int, 0, len(scyllaPods))
	...
	for _, scyllaPod := range scyllaPods {
		...
		scyllaProcessHostPIDs = append(scyllaProcessHostPIDs, *containerStatus.Info.PID)
	}
	...
	if len(scyllaProcessHostPIDs) == 0 {
		klog.Errorf("can't determine Scylla container process PIDs from CRI, their resource process limits won't be raised")
		return nil, nil
	}

	rlimitsJobs, err := makeRlimitsJobForContainers(cr, ncdc.namespace, ncdc.nodeConfigName, ncdc.nodeName, ncdc.nodeUID, ncdc.operatorImage, podSpec, scyllaContainerIDs, scyllaProcessHostPIDs)
	if err != nil {
...
func (ncdc *Controller) makeJobsForContainers(ctx context.Context) ([]*batchv1.Job, error) {
	...
	// Sort Pods to have stable representation for the same set of Pods.
	...
	var runningScyllaContainerIDs []string
	...
	for _, scyllaPod := range localScyllaPods {
		...
		runningScyllaContainerIDs = append(runningScyllaContainerIDs, containerID)
	...
	resourceLimitsJob, err := ncdc.makeResourceLimitJobsForContainers(ctx, &selfPod.Spec, runningScyllaPods, runningScyllaContainerIDs)
	if err != nil {
	...
	if resourceLimitsJob != nil {
		containerJobs = append(containerJobs, resourceLimitsJob)
	}
...
func (ncdc *Controller) syncJobs(ctx context.Context, jobs map[string]*batchv1.Job, nodeStatus *scyllav1alpha1.NodeConfigNodeStatus) error {
	...
	requiredJobs := make([]*batchv1.Job, 0, len(requiredForNode)+2)
