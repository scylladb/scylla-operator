...
var (
	keyFunc = controllerhelpers.DeletionHandlingTypedKeyFunc
)
...
func (opc *Controller) updateNode(old, cur interface{}) {
	...
	if currentNode.UID != oldNode.UID {
		...
		opc.deleteNode(controllerhelpers.DeletedStronglyTypedFinalStateUnknown{
			Key: key,
			Obj: oldNode,
		})
	}
...
func (opc *Controller) deleteNode(obj interface{}) {
	...
	if !ok {
		tombstone, ok := obj.(controllerhelpers.DeletedStronglyTypedFinalStateUnknown)
		if !ok {
...
func (opc *Controller) updateScyllaDBDatacenter(old, cur interface{}) {
	...
	if currentSDC.UID != oldSDC.UID {
		...
		opc.deleteScyllaDBDatacenter(controllerhelpers.DeletedStronglyTypedFinalStateUnknown{
			Key: key,
			Obj: oldSDC,
		})
	}
