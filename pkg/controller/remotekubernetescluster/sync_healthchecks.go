...
func (rkcc *Controller) syncClientHealthchecks(ctx context.Context, rq *controllertools.Requeue, rkc *scyllav1alpha1.RemoteKubernetesCluster, status *scyllav1alpha1.RemoteKubernetesClusterStatus) ([]metav1.Condition, error) {
	...
	rq.After(time.Duration(rkc.Spec.ClientHealthcheckProbes.PeriodSeconds) * time.Second)
