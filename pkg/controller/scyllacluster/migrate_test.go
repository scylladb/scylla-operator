	...
	tt := []struct {
		...
		},
		{
			...
			expectedScyllaDBDatacenter: func() *scyllav1alpha1.ScyllaDBDatacenter {
				...
				metav1.SetMetaDataAnnotation(&sd.ObjectMeta, "internal.scylla-operator.scylladb.com/host-networking", "true")
				return sd
			}(),
		},
		{
			...
			expectedScyllaDBDatacenter: func() *scyllav1alpha1.ScyllaDBDatacenter {
				sd := newBasicScyllaDBDatacenterWithNoStatus()
				metav1.SetMetaDataAnnotation(&sd.ObjectMeta, "internal.scylla-operator.scylladb.com/sysctls", "[\"foo=bar\",\"zoo=foo\"]\n")
				return sd
			}(),
		},
		{
			...
			expectedScyllaDBDatacenter: func() *scyllav1alpha1.ScyllaDBDatacenter {
				sd := newBasicScyllaDBDatacenterWithNoStatus()
				metav1.SetMetaDataAnnotation(&sd.ObjectMeta, "internal.scylla-operator.scylladb.com/alternator-port", "9000")
				return sd
			}(),
		},
		{
			...
			expectedScyllaDBDatacenter: func() *scyllav1alpha1.ScyllaDBDatacenter {
				sd := newBasicScyllaDBDatacenterWithNoStatus()
				metav1.SetMetaDataAnnotation(&sd.ObjectMeta, "internal.scylla-operator.scylladb.com/alternator-insecure-enable-http", "true")
				return sd
			}(),
		},
		{
			...
			expectedScyllaDBDatacenter: func() *scyllav1alpha1.ScyllaDBDatacenter {
				sd := newBasicScyllaDBDatacenterWithNoStatus()
				metav1.SetMetaDataAnnotation(&sd.ObjectMeta, "internal.scylla-operator.scylladb.com/alternator-insecure-disable-authorization", "true")
				return sd
			}(),
		},
		{
...
func newBasicScyllaDBDatacenter() *scyllav1alpha1.ScyllaDBDatacenter {
	return &scyllav1alpha1.ScyllaDBDatacenter{
		ObjectMeta: metav1.ObjectMeta{
			...
			Namespace: "scylla",
			Labels: map[string]string{
				"scylla-operator.scylladb.com/register-with-global-scylladb-manager": "true",
			},
			Annotations: map[string]string{
				"internal.scylla-operator.scylladb.com/scylladb-manager-cluster-name-override": "scylla/simple-cluster",
			},
		},
...
func TestMigrateV1ScyllaClusterToV1Alpha1ScyllaDBManagerTasks(t *testing.T) {
	...
	tt := []struct {
		...
		},
		{
			...
			expectedScyllaDBManagerTasks: []*scyllav1alpha1.ScyllaDBManagerTask{
				newBackupScyllaDBManagerTask(),
				newRepairScyllaDBManagerTask(),
			},
			expectedErr: nil,
		},
	}
