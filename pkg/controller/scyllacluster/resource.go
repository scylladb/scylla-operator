...
func MakeEndpointSlices(sc *scyllav1.ScyllaCluster, services map[string]*corev1.Service, podLister corev1listers.PodLister) ([]*discoveryv1.EndpointSlice, error) {
	...
	for _, svc := range services {
		...
		for _, servicePort := range svc.Spec.Ports {
			...
			containerServingPort, ok, err := controllerhelpers.FindContainerServingPort(servicePort, pod.Spec.Containers)
			if err != nil {
				return endpointSlices, fmt.Errorf("can't find container serving port: %w", err)
			}
