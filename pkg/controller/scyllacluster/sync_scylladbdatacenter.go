...
func (scmc *Controller) syncScyllaDBDatacenter(ctx context.Context, sc *scyllav1.ScyllaCluster, sdcs map[string]*scyllav1alpha1.ScyllaDBDatacenter) ([]metav1.Condition, error) {
	...
	if upgradeContext != nil && !ok {
		...
		existingSDc, err = ctrlclient.Get[scyllav1alpha1.ScyllaDBDatacenter](ctx, scmc.apiReader, sc.Namespace, sc.Name)
		...
		if existingSDc == nil {
			...
			_, changed, err := resourceapply.ApplyConfigMapWithControl(ctx, ctrlclient.ApplyControl[corev1.ConfigMap](ctx, scmc.client, sc.Namespace), scmc.eventRecorder, ucCM, resourceapply.ApplyOptions{
	...
	_, changed, err := resourceapply.ApplyScyllaDBDatacenterWithControl(ctx, ctrlclient.ApplyControl[scyllav1alpha1.ScyllaDBDatacenter](ctx, scmc.client, sc.Namespace), scmc.eventRecorder, sdc, resourceapply.ApplyOptions{})
	if changed {
		controllerhelpers.AddGenericProgressingStatusCondition(&progressingConditions, scyllaDBDatacenterControllerProgressingCondition, sdc, "apply", sdc.Generation)
	}
	if err != nil {
