...
func (scc *Controller) makeRacks(sdc *scyllav1alpha1.ScyllaDBDatacenter, statefulSets map[string]*appsv1.StatefulSet, inputsHash string) ([]*appsv1.StatefulSet, error) {
	...
	for i, rack := range sdc.Spec.Racks {
		...
		sts, err := StatefulSetForRack(rack, sdc, oldSts, scc.operatorImage, i, inputsHash)
		if err != nil {
			return nil, err
		}
...
func (scc *Controller) getScyllaManagerAgentToken(ctx context.Context, sdc *scyllav1alpha1.ScyllaDBDatacenter) (string, error) {
...
func (scc *Controller) getScyllaClient(ctx context.Context, sdc *scyllav1alpha1.ScyllaDBDatacenter, hosts []string) (*scyllaclient.Client, error) {
	managerAgentAuthToken, err := scc.getScyllaManagerAgentToken(ctx, sdc)
	if err != nil {
...
func (scc *Controller) beforeUpgrade(ctx context.Context, sdc *scyllav1alpha1.ScyllaDBDatacenter, services map[string]*corev1.Service, upgradeContext *upgradeContext) (bool, error) {
	...
	hosts, err := controllerhelpers.GetRequiredScyllaHosts(sdc, services, scc.podLister)
	if err != nil {
		return true, err
	}

	scyllaClient, err := scc.getScyllaClient(ctx, sdc, hosts)
	if err != nil {
		return true, err
	}
	...
	err = scc.backupKeyspaces(ctx, scyllaClient, hosts, systemKeyspaces, upgradeContext.SystemSnapshotTag)
	if err != nil {
		return true, err
	}
...
func (scc *Controller) afterUpgrade(ctx context.Context, sdc *scyllav1alpha1.ScyllaDBDatacenter, services map[string]*corev1.Service, upgradeContext *upgradeContext) error {
	...
	// Clear system backup.
	err = scc.removeSnapshot(ctx, scyllaClient, hosts, []string{upgradeContext.SystemSnapshotTag})
	if err != nil {
		return err
	}
...
// It returns true if the action is done, false if the caller should repeat later.
func (scc *Controller) beforeNodeUpgrade(ctx context.Context, sdc *scyllav1alpha1.ScyllaDBDatacenter, sts *appsv1.StatefulSet, ordinal int32, services map[string]*corev1.Service, upgradeContext *upgradeContext) (bool, error) {
	...
	pod, err := scc.podLister.Pods(sdc.Namespace).Get(podName)
	if err != nil {
	...
	scyllaClient, err := scc.getScyllaClient(ctx, sdc, []string{host})
	if err != nil {
		return true, err
	}
	defer scyllaClient.Close()
	...
	err = scc.backupKeyspaces(ctx, scyllaClient, []string{host}, keyspaceSet.List(), upgradeContext.DataSnapshotTag)
	if err != nil {
		return true, err
	}
	...
	err = scc.kubeClient.CoreV1().Pods(sdc.Namespace).Delete(ctx, podName, metav1.DeleteOptions{})
	if err != nil {
...
func (scc *Controller) afterNodeUpgrade(ctx context.Context, sdc *scyllav1alpha1.ScyllaDBDatacenter, sts *appsv1.StatefulSet, ordinal int32, services map[string]*corev1.Service, upgradeContext *upgradeContext) error {
	svcName := fmt.Sprintf("%s-%d", sts.Name, ordinal)
	svc, ok := services[svcName]
	if !ok {
	...
	// Clear data backup.
	err = scc.removeSnapshot(ctx, scyllaClient, []string{host}, []string{upgradeContext.DataSnapshotTag})
	if err != nil {
		return err
	}
...
func (scc *Controller) pruneStatefulSets(
	...
	for _, sts := range statefulSets {
		...
		if ok {
			status.Racks = append(status.Racks[:idx], status.Racks[idx+1:]...)
		}
	}
	...
	for _, req := range requiredStatefulSets {
		...
		if !found {
			...
			if changed {
				...
				updatedRackStatus := *scc.calculateRackStatus(sdc, sts)
				_, idx, ok := slices.Find(status.Racks, func(rackStatus scyllav1alpha1.RackStatus) bool {
					return rackStatus.Name == rackName
				})
				if ok {
...
func (scc *Controller) syncStatefulSets(
	...
	requiredStatefulSets, err := scc.makeRacks(sdc, statefulSets, inputsHash)
	if err != nil {
	...
	pruneProgressingConditions, err := scc.pruneStatefulSets(ctx, sdc, status, requiredStatefulSets, statefulSets)
	...
	createProgressingConditions, err := scc.createMissingStatefulSets(ctx, sdc, status, requiredStatefulSets, statefulSets, services)
	...
	for _, req := range requiredStatefulSets {
		...
		for _, svc := range rackServices {
			if svc.Labels[naming.DecommissionedLabel] == naming.LabelValueFalse {
				...
				// TODO: move do migrating controller
				// rackName, ok := sts.Labels[naming.RackNameLabel]
	...
	if ok {
		currentUpgradeContext, err := scc.decodeUpgradeContext(upgradeContextConfigMap)
		if err != nil {
		...
		// Isolate the live values in a block to prevent accidental use.
		{
			...
			freshUpgradeContextConfigMap, err := scc.kubeClient.CoreV1().ConfigMaps(sdc.Namespace).Get(ctx, naming.UpgradeContextConfigMapName(sdc), metav1.GetOptions{})
			if err != nil {
			...
			freshUpgradeContext, err := scc.decodeUpgradeContext(freshUpgradeContextConfigMap)
			if err != nil {
				return progressingConditions, fmt.Errorf("can't decode upgrade context for ScyllaDBDatacenter %q: %w", naming.ObjRef(sdc), err)
			}
		...
		case PreHooksUpgradePhase:
			// TODO: Move the pre-upgrade hook into a Job.
			done, err := scc.beforeUpgrade(ctx, sdc, services, currentUpgradeContext)
			if err != nil {
				return progressingConditions, err
			}
			if !done {
			...
			cm, changed, err := resourceapply.ApplyConfigMap(ctx, scc.kubeClient.CoreV1(), scc.configMapLister, scc.eventRecorder, cm, resourceapply.ApplyOptions{})
			if changed {
		...
		case RolloutInitUpgradePhase:
			...
			for _, required := range requiredStatefulSets {
				...
				if changed {
					...
					status.Racks[idx] = *scc.calculateRackStatus(sdc, updatedSts)
				}
			}
			if anyStsChanged {
				// TODO: Add expectations, not to reconcile sooner then we see this new StatefulSet in our caches. (#682)
		...
		case RolloutRunUpgradePhase:
			for _, sts := range requiredStatefulSets {
				...
				if partition < *sts.Spec.Replicas {
					...
					err = scc.afterNodeUpgrade(ctx, sdc, sts, partition, services, currentUpgradeContext)
					if err != nil {
						return progressingConditions, err
					}
				...
				done, err := scc.beforeNodeUpgrade(ctx, sdc, sts, nextPartition, services, currentUpgradeContext)
				if err != nil {
					return progressingConditions, err
				}
		...
		case PostHooksUpgradePhase:
			err = scc.afterUpgrade(ctx, sdc, services, currentUpgradeContext)
			if err != nil {
				return progressingConditions, err
			}

			cmName := naming.UpgradeContextConfigMapName(sdc)
			err := scc.kubeClient.CoreV1().ConfigMaps(sdc.Namespace).Delete(ctx, cmName, metav1.DeleteOptions{})
			if err != nil {
		...
		if existingFound && upgradeContextConfigMap == nil {
			...
			if requiredVersionLabelPresent && existingVersionLabelPresent {
				...
				if requiredVersion.Major != existingVersion.Major ||
					...
					scc.eventRecorder.Eventf(sdc, corev1.EventTypeNormal, "UpgradeStarted", "Version changed from %q to %q", existingVersionString, requiredVersionString)
...
func (scc *Controller) decodeUpgradeContext(upgradeContextConfigMap *corev1.ConfigMap) (*upgradeContext, error) {
	...
	uc := &upgradeContext{}
	err := json.NewDecoder(strings.NewReader(ucRaw)).Decode(uc)
	if err != nil {
...
func makeUpgradeContextConfigMap(sdc *scyllav1alpha1.ScyllaDBDatacenter, uc *upgradeContext) (*corev1.ConfigMap, error) {
	...
	return &corev1.ConfigMap{
		...
		Data: map[string]string{
			naming.UpgradeContextConfigMapKey: buf.String(),
		},
	}, nil
}
