...
func (scmc *Controller) addService(obj interface{}) {
	scmc.handlers.HandleAdd(
		obj.(*corev1.Service),
		scmc.enqueueIfScyllaClusterIsOwner(),
	)
}
...
func (scmc *Controller) enqueueIfScyllaClusterIsOwner() controllerhelpers.EnqueueFuncType {
	return func(depth int, obj kubeinterfaces.ObjectInterface, operation controllerhelpers.HandlerOperationType) {
		...
		owner, err := scmc.scyllaClusterLister.ScyllaClusters(obj.GetNamespace()).Get(controllerRef.Name)
		if err != nil {
		...
		klog.V(4).InfoSDepth(depth, "Enqueuing owner", []any{"GVK", resource.GetObjectGVKOrUnknown(owner), "Ref", klog.KObj(owner), "UID", owner.GetUID()}...)
		scmc.handlers.Enqueue(depth+1, owner, operation)
	}
}
