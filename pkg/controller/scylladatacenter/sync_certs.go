...
func makeScyllaConnectionConfig(
	...
	clientCertSecretName := naming.GetScyllaClusterLocalUserAdminCertName(sd.Name)
	clientCertSecret, found := secrets[clientCertSecretName]
	if !found {
		return nil, fmt.Errorf("secret %q doesn't exist or is not own by this object", naming.ManualRef(sd.Namespace, clientCertSecretName))
	}
	...
	servingCAConfigMapName := naming.GetScyllaClusterLocalServingCAName(sd.Name)
	servingCAConfigMap, found := configMaps[servingCAConfigMapName]
	if !found {
		return nil, fmt.Errorf("configmap %q doesn't exist or is not own by this object", naming.ManualRef(sd.Namespace, servingCAConfigMapName))
	}
	...
	secret := &corev1.Secret{
		ObjectMeta: metav1.ObjectMeta{
			...
			Name:      naming.GetScyllaClusterLocalAdminCQLConnectionConfigsName(sd.Name),
			Labels:    naming.ClusterLabels(sd),
			OwnerReferences: []metav1.OwnerReference{
				*metav1.NewControllerRef(sd, scyllaDatacenterControllerGVK),
			},
		},
		Data: make(map[string][]byte, len(sd.Spec.DNSDomains)),
	...
	for _, domain := range sd.Spec.DNSDomains {
		scyllaConnectionConfig := &cqlclientv1alpha1.CQLConnectionConfig{
			...
			Contexts: map[string]*cqlclientv1alpha1.Context{
				"default": {
					AuthInfoName:   "admin",
					DatacenterName: sd.Spec.DatacenterName,
				},
			},
...
func (scc *Controller) syncCerts(
	...
	clusterLabels := naming.ClusterLabels(sd)
	...
	errors = append(errors, cm.ManageCertificates(
		...
		&okubecrypto.CAConfig{
			MetaConfig: okubecrypto.MetaConfig{
				Name:   naming.GetScyllaClusterLocalClientCAName(sd.Name),
		...
		[]*okubecrypto.CertificateConfig{
			{
				MetaConfig: okubecrypto.MetaConfig{
					Name:   naming.GetScyllaClusterLocalUserAdminCertName(sd.Name),
					Labels: clusterLabels,
				},
				Validity: 10 * 365 * 24 * time.Hour,
				Refresh:  8 * 365 * 24 * time.Hour,
	...
	errors = append(errors, cm.ManageCertificates(
		...
		&okubecrypto.CAConfig{
			MetaConfig: okubecrypto.MetaConfig{
				Name:   naming.GetScyllaClusterLocalServingCAName(sd.Name),
				Labels: clusterLabels,
			},
			Validity: 10 * 365 * 24 * time.Hour,
			Refresh:  8 * 365 * 24 * time.Hour,
		...
		[]*okubecrypto.CertificateConfig{
			{
				MetaConfig: okubecrypto.MetaConfig{
					Name:   naming.GetScyllaClusterLocalServingCertName(sd.Name),
					Labels: clusterLabels,
				},
	...
	scyllaConnectionConfigSecret, err := makeScyllaConnectionConfig(sd, secrets, configMaps, scc.cqlsIngressPort)
	if err != nil {
		...
		if changed {
			controllerhelpers.AddGenericProgressingStatusCondition(&progressingConditions, certControllerProgressingCondition, scyllaConnectionConfigSecret, "apply", sd.Generation)
		}
		if err != nil {
