	...
	for _, otherDC := range sc.Spec.Datacenters {
		...
		seedServices = append(seedServices, &corev1.Service{
			...
			Spec: corev1.ServiceSpec{
				Ports: slices.ConvertSlice(scyllaDBInterNodeCommunicationPorts, func(port portSpec) corev1.ServicePort {
	...
	for _, otherDC := range sc.Spec.Datacenters {
		...
		if !ok {
			progressingConditions = append(progressingConditions, metav1.Condition{
				Type:               fmt.Sprintf(remoteEndpointSliceControllerDatacenterProgressingConditionFormat, dc.Name),
		...
		endpoints, err := calculateEndpointsForOtherDCPods(sc, nodeBroadcastType, otherDC, otherDCNamespace, otherDCPodSelector, remotePodLister, remoteServiceLister)
		if err != nil {
		...
		remoteEndpointSlices = append(remoteEndpointSlices, &discoveryv1.EndpointSlice{
			...
			Endpoints:   endpoints,
			Ports: slices.ConvertSlice(scyllaDBInterNodeCommunicationPorts, func(port portSpec) discoveryv1.EndpointPort {
...
// calculateEndpointsForOtherDCPods computes endpoints for other datacenter pods taking into account how nodes are being exposed.
func calculateEndpointsForOtherDCPods(sc *scyllav1alpha1.ScyllaDBCluster, nodeBroadcastType scyllav1alpha1.BroadcastAddressType, otherDC scyllav1alpha1.ScyllaDBClusterDatacenter, otherDCNamespace *corev1.Namespace, otherDCPodSelector labels.Selector, remotePodLister remotelister.GenericClusterLister[corev1listers.PodLister], remoteServiceLister remotelister.GenericClusterLister[corev1listers.ServiceLister]) ([]discoveryv1.Endpoint, error) {
	...
	case scyllav1alpha1.BroadcastAddressTypePodIP:
		...
		if err != nil {
			return nil, fmt.Errorf("can't list pods in %q ScyllaCluster %q Datacenter: %w", naming.ObjRef(sc), otherDC.Name, err)
		}
	...
	case scyllav1alpha1.BroadcastAddressTypeServiceLoadBalancerIngress:
		...
		if err != nil {
			return nil, fmt.Errorf("can't list services in %q ScyllaDBCluster %q Datacenter: %w", naming.ObjRef(sc), otherDC.Name, err)
		}
	...
	for _, cmName := range configMapsToMirror {
		...
		if err != nil {
			if apierrors.IsNotFound(err) {
				progressingConditions = append(progressingConditions, metav1.Condition{
					Type:               fmt.Sprintf(remoteConfigMapControllerDatacenterProgressingConditionFormat, dc.Name),
					Status:             metav1.ConditionTrue,
...
func makeMirroredRemoteSecrets(sc *scyllav1alpha1.ScyllaDBCluster, dc *scyllav1alpha1.ScyllaDBClusterDatacenter, remoteNamespace *corev1.Namespace, remoteController metav1.Object, localSecretLister corev1listers.SecretLister, managingClusterDomain string) ([]metav1.Condition, []*corev1.Secret, error) {
	...
	for _, secretName := range secretsToMirror {
		...
		if err != nil {
			if apierrors.IsNotFound(err) {
				progressingConditions = append(progressingConditions, metav1.Condition{
					Type:               fmt.Sprintf(remoteSecretControllerDatacenterProgressingConditionFormat, dc.Name),
					Status:             metav1.ConditionTrue,
