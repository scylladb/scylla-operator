...
func Test_getSecretsToMirrorForAllDCs(t *testing.T) {
	...
	tt := []struct {
		...
		{
			name: "Mandatory secrets to mirror",
			sc:   newBasicScyllaDBCluster(),
			expected: []string{
				"cluster-auth-token-2s75a",
			},
			expectedErr: nil,
		},
		{
			...
			sc: func() *scyllav1alpha1.ScyllaDBCluster {
				...
				if sc.Spec.DatacenterTemplate.ScyllaDBManagerAgent == nil {
					sc.Spec.DatacenterTemplate.ScyllaDBManagerAgent = &scyllav1alpha1.ScyllaDBManagerAgentTemplate{}
				}
				sc.Spec.DatacenterTemplate.ScyllaDBManagerAgent.CustomConfigSecretRef = pointer.Ptr("scyllaDBManagerAgent.customConfigSecretRef")

				if sc.Spec.DatacenterTemplate.RackTemplate == nil {
					sc.Spec.DatacenterTemplate.RackTemplate = &scyllav1alpha1.RackTemplate{}
				}
				if sc.Spec.DatacenterTemplate.RackTemplate.ScyllaDBManagerAgent == nil {
