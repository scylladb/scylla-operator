...
func (scc *Controller) syncRemoteConfigMaps(
	...
	for _, cm := range requiredConfigMaps {
		...
		if changed {
			controllerhelpers.AddGenericProgressingStatusCondition(&progressingConditions, fmt.Sprintf(remoteConfigMapControllerDatacenterProgressingConditionFormat, dc.Name), cm, "apply", sc.Generation)
		}
		if err != nil {
