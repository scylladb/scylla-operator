...
func (scc *Controller) syncRemoteEndpoints(
	...
	for _, e := range requiredEndpoints {
		...
		if changed {
			controllerhelpers.AddGenericProgressingStatusCondition(&progressingConditions, fmt.Sprintf(remoteEndpointsControllerDatacenterProgressingConditionFormat, dc.Name), e, "apply", sc.Generation)
		}
		if err != nil {
