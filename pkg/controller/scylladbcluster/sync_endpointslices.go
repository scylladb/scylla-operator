...
func (scc *Controller) syncRemoteEndpointSlices(
	...
	for _, es := range requiredEndpointSlices {
		...
		if changed {
			controllerhelpers.AddGenericProgressingStatusCondition(&progressingConditions, fmt.Sprintf(remoteEndpointSliceControllerDatacenterProgressingConditionFormat, dc.Name), es, "apply", sc.Generation)
		}
		if err != nil {
