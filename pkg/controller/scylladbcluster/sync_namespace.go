...
func (scc *Controller) syncRemoteNamespaces(
	...
	for _, rns := range requiredNamespaces {
		...
		if changed {
			controllerhelpers.AddGenericProgressingStatusCondition(&progressingConditions, fmt.Sprintf(remoteNamespaceControllerDatacenterProgressingConditionFormat, dc.Name), rns, "apply", sc.Generation)
		}
		if err != nil {
