...
func (scc *Controller) syncRemoteRemoteOwners(
	...
	for _, ro := range requiredRemoteOwners {
		...
		if changed {
			controllerhelpers.AddGenericProgressingStatusCondition(&progressingConditions, fmt.Sprintf(remoteRemoteOwnerControllerDatacenterProgressingConditionFormat, dc.Name), ro, "apply", sc.Generation)
		}
		if err != nil {
