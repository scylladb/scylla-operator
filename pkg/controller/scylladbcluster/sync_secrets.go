...
func (scc *Controller) syncRemoteSecrets(
	...
	for _, rs := range requiredRemoteSecrets {
		...
		if changed {
			controllerhelpers.AddGenericProgressingStatusCondition(&progressingConditions, fmt.Sprintf(remoteSecretControllerDatacenterProgressingConditionFormat, dc.Name), rs, "apply", sc.Generation)
		}
		if err != nil {
