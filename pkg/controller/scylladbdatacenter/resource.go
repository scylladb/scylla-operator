...
// It always returns a non-nil NodeStatusReport for the ordinal, unless an error occurs.
// HostID is nil when the node has no durable ScyllaDB identity yet (e.g. still bootstrapping/joining).
func makeNodeStatusReport(sdc *scyllav1alpha1.ScyllaDBDatacenter, rackSpec *scyllav1alpha1.RackSpec, ordinal int, services map[string]*corev1.Service, podLister corev1listers.PodLister) (*scyllav1alpha1.NodeStatusReport, error) {
	...
	svc, ok := services[svcName]
	if !ok {
		klog.V(5).InfoS("Service is missing, reporting node with no known identity", "ScyllaDBDatacenter", klog.KObj(sdc), "Service", klog.KRef(sdc.Namespace, svcName))
		return nodeStatusReport, nil
	}

	hostID := svc.Annotations[naming.HostIDAnnotation]
	if len(hostID) == 0 {
		klog.V(5).InfoS("Node has no known identity in ScyllaDB, reporting node with no known identity", "ScyllaDBDatacenter", klog.KObj(sdc), "Service", klog.KObj(svc))
		return nodeStatusReport, nil
	}
	...
	if err != nil {
		...
		// Pod is missing for a node with a known identity in ScyllaDB, report no observed nodes.
		klog.V(4).InfoS("Pod of a node with known identity in ScyllaDB is missing, reporting no observed nodes", "ScyllaDBDatacenter", klog.KObj(sdc), "Service", klog.KRef(sdc.Namespace, svcName), "Pod", klog.KRef(sdc.Namespace, podName))
		return nodeStatusReport, nil
	}

	nodeStatusReportAnnotationValue, ok := pod.Annotations[naming.NodeStatusReportAnnotation]
	if !ok {
		// The node might not have reported its status yet, report no observed nodes.
		klog.V(4).InfoS("Node status report annotation is missing on Pod of a node with known identity in ScyllaDB, reporting no observed nodes", "ScyllaDBDatacenter", klog.KObj(sdc), "Service", klog.KRef(sdc.Namespace, svcName), "Pod", klog.KObj(pod), "AnnotationKey", naming.NodeStatusReportAnnotation)
		return nodeStatusReport, nil
	}
	...
	if internalNodeStatusReport.Error != nil {
		// The node reported an error, report no observed nodes.
		klog.V(4).InfoS("Node reported an error in its status report, reporting no observed nodes", "ScyllaDBDatacenter", klog.KObj(sdc), "Service", klog.KRef(sdc.Namespace, svcName), "Pod", klog.KObj(pod), "Error", internalNodeStatusReport.Error)
		return nodeStatusReport, nil
	}
	...
	klog.V(5).InfoS("Successfully built a node status report", "ScyllaDBDatacenter", klog.KObj(sdc), "Service", klog.KRef(sdc.Namespace, svcName), "Pod", klog.KObj(pod))
	return nodeStatusReport, nil
}
