...
func (sdcc *Controller) Reconcile(ctx context.Context, req reconcile.Request) (reconcile.Result, error) {
	if sdcc.reconcileObserver != nil {
		sdcc.reconcileObserver(req.NamespacedName)
	}
	...
	sdc, err := ctrlclient.Get[scyllav1alpha1.ScyllaDBDatacenter](ctx, sdcc.client.Client(), namespace, name)
	...
	soc, err := ctrlclient.Get[scyllav1alpha1.ScyllaOperatorConfig](ctx, sdcc.client.Client(), "", naming.SingletonName)
	if err != nil {
	...
	statefulSetMap, err := controllerhelpers.GetObjects[CT, *appsv1.StatefulSet](
		...
		sdcSelector,
		ctrlclient.GetObjectsControl[scyllav1alpha1.ScyllaDBDatacenter, appsv1.StatefulSet](ctx, sdcc.client.Client(), sdcc.apiReader, sdc.Namespace),
	)
	if err != nil {
	...
	serviceMap, err := controllerhelpers.GetObjects[CT, *corev1.Service](
		...
		scyllav1alpha1.ScyllaDBDatacenterGVK,
		sdcSelector,
		ctrlclient.GetObjectsControl[scyllav1alpha1.ScyllaDBDatacenter, corev1.Service](ctx, sdcc.client.Client(), sdcc.apiReader, sdc.Namespace),
	)
	if err != nil {
		objectErrs = append(objectErrs, err)
	}
	...
	secretMap, err := controllerhelpers.GetObjects[CT, *corev1.Secret](
		...
		scyllav1alpha1.ScyllaDBDatacenterGVK,
		sdcSelector,
		ctrlclient.GetObjectsControl[scyllav1alpha1.ScyllaDBDatacenter, corev1.Secret](ctx, sdcc.client.Client(), sdcc.apiReader, sdc.Namespace),
	)
	if err != nil {
		objectErrs = append(objectErrs, err)
	}
	...
	configMapMap, err := controllerhelpers.GetObjects[CT, *corev1.ConfigMap](
		...
		scyllav1alpha1.ScyllaDBDatacenterGVK,
		sdcSelector,
		ctrlclient.GetObjectsControl[scyllav1alpha1.ScyllaDBDatacenter, corev1.ConfigMap](ctx, sdcc.client.Client(), sdcc.apiReader, sdc.Namespace),
	)
	if err != nil {
		objectErrs = append(objectErrs, err)
	}
	...
	serviceAccounts, err := controllerhelpers.GetObjects[CT, *corev1.ServiceAccount](
		...
		scyllav1alpha1.ScyllaDBDatacenterGVK,
		sdcSelector,
		ctrlclient.GetObjectsControl[scyllav1alpha1.ScyllaDBDatacenter, corev1.ServiceAccount](ctx, sdcc.client.Client(), sdcc.apiReader, sdc.Namespace),
	)
	if err != nil {
		objectErrs = append(objectErrs, err)
	}
	...
	roleBindings, err := controllerhelpers.GetObjects[CT, *rbacv1.RoleBinding](
		...
		scyllav1alpha1.ScyllaDBDatacenterGVK,
		sdcSelector,
		ctrlclient.GetObjectsControl[scyllav1alpha1.ScyllaDBDatacenter, rbacv1.RoleBinding](ctx, sdcc.client.Client(), sdcc.apiReader, sdc.Namespace),
	)
	if err != nil {
		objectErrs = append(objectErrs, err)
	}
	...
	pdbMap, err := controllerhelpers.GetObjects[CT, *policyv1.PodDisruptionBudget](
		...
		scyllav1alpha1.ScyllaDBDatacenterGVK,
		sdcSelector,
		ctrlclient.GetObjectsControl[scyllav1alpha1.ScyllaDBDatacenter, policyv1.PodDisruptionBudget](ctx, sdcc.client.Client(), sdcc.apiReader, sdc.Namespace),
	)
	if err != nil {
		objectErrs = append(objectErrs, err)
	}
	...
	ingressMap, err := controllerhelpers.GetObjects[CT, *networkingv1.Ingress](
		...
		scyllav1alpha1.ScyllaDBDatacenterGVK,
		sdcSelector,
		ctrlclient.GetObjectsControl[scyllav1alpha1.ScyllaDBDatacenter, networkingv1.Ingress](ctx, sdcc.client.Client(), sdcc.apiReader, sdc.Namespace),
	)
	if err != nil {
		objectErrs = append(objectErrs, err)
	}
	...
	jobMap, err := controllerhelpers.GetObjects[CT, *batchv1.Job](
		...
		scyllav1alpha1.ScyllaDBDatacenterGVK,
		sdcSelector,
		ctrlclient.GetObjectsControl[scyllav1alpha1.ScyllaDBDatacenter, batchv1.Job](ctx, sdcc.client.Client(), sdcc.apiReader, sdc.Namespace),
	)
	if err != nil {
		objectErrs = append(objectErrs, err)
	}
	...
	scyllaDBDatacenterNodesStatusReportMap, err := controllerhelpers.GetObjects[CT, *scyllav1alpha1.ScyllaDBDatacenterNodesStatusReport](
		...
		scyllav1alpha1.ScyllaDBDatacenterGVK,
		sdcSelector,
		ctrlclient.GetObjectsControl[scyllav1alpha1.ScyllaDBDatacenter, scyllav1alpha1.ScyllaDBDatacenterNodesStatusReport](ctx, sdcc.client.Client(), sdcc.apiReader, sdc.Namespace),
	)
	if err != nil {
		objectErrs = append(objectErrs, err)
	}
	...
	// Don't return a requeue together with an error: controller-runtime ignores the result of a failed
	// reconciliation, requeues it rate-limited instead, and logs a warning about the result it ignored.
