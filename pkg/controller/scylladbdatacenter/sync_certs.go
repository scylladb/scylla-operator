...
func (sdcc *Controller) syncCerts(
	...
	if utilfeature.DefaultMutableFeatureGate.Enabled(features.AutomaticTLSCertificates) || sdc.Spec.ScyllaDB.AlternatorOptions != nil {
		for _, svc := range serviceMap {
			...
			pod, err := ctrlclient.Get[corev1.Pod](ctx, sdcc.client.Client(), sdc.Namespace, svc.Name)
			if err != nil {
	...
	if utilfeature.DefaultMutableFeatureGate.Enabled(features.AutomaticTLSCertificates) {
		...
		if err != nil {
			...
			_, changed, err := resourceapply.ApplySecretWithControl(ctx, ctrlclient.ApplyControl[corev1.Secret](ctx, sdcc.client.Client(), sdc.Namespace), sdcc.eventRecorder, scyllaConnectionConfigSecret, resourceapply.ApplyOptions{})
			if changed {
