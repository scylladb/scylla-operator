...
func (sdcc *Controller) syncConfigs(
	...
	for _, cm := range requiredConfigMaps {
		_, changed, err := resourceapply.ApplyConfigMapWithControl(ctx, ctrlclient.ApplyControl[corev1.ConfigMap](ctx, sdcc.client.Client(), sdc.Namespace), sdcc.eventRecorder, cm, resourceapply.ApplyOptions{})
		if changed {
