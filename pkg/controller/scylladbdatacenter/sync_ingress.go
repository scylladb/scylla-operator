...
func (sdcc *Controller) syncIngresses(
	...
	for _, ingress := range ingresses {
		...
		err = sdcc.client.Client().Delete(ctx, ingress, client.Preconditions{UID: &ingress.UID}, client.PropagationPolicy(propagationPolicy))
	...
	for _, requiredIngress := range requiredIngresses {
		_, changed, err := resourceapply.ApplyIngressWithControl(ctx, ctrlclient.ApplyControl[networkingv1.Ingress](ctx, sdcc.client.Client(), sdc.Namespace), sdcc.eventRecorder, requiredIngress, resourceapply.ApplyOptions{})
		if changed {
