...
func (sdcc *Controller) syncJobs(
	...
	err = controllerhelpers.Prune(ctx, requiredJobs, jobs,
		&controllerhelpers.PruneControlFuncs{
			DeleteFunc: ctrlclient.DeleteFunc[batchv1.Job](sdcc.client.Client(), sdc.Namespace),
		},
		sdcc.eventRecorder,
	)
	if err != nil {
	...
	for _, job := range requiredJobs {
		fresh, changed, err := resourceapply.ApplyJobWithControl(ctx, ctrlclient.ApplyControl[batchv1.Job](ctx, sdcc.client.Client(), sdc.Namespace), sdcc.eventRecorder, job, resourceapply.ApplyOptions{})
		if changed {
