...
func (sdcc *Controller) syncPodDisruptionBudgets(
	...
	for _, pdb := range pdbs {
		...
		err = sdcc.client.Client().Delete(ctx, pdb, client.Preconditions{UID: &pdb.UID}, client.PropagationPolicy(propagationPolicy))
	...
	_, changed, err := resourceapply.ApplyPodDisruptionBudgetWithControl(ctx, ctrlclient.ApplyControl[policyv1.PodDisruptionBudget](ctx, sdcc.client.Client(), sdc.Namespace), sdcc.eventRecorder, requiredPDB, resourceapply.ApplyOptions{
		ForceOwnership: true,
	})
	if changed {
