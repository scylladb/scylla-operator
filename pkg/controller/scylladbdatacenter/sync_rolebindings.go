...
func (sdcc *Controller) syncRoleBindings(
	...
	for _, rb := range roleBindings {
		...
		err = sdcc.client.Client().Delete(ctx, rb, client.Preconditions{UID: &rb.UID}, client.PropagationPolicy(propagationPolicy))
	...
	_, changed, err := resourceapply.ApplyRoleBindingWithControl(ctx, ctrlclient.ApplyControl[rbacv1.RoleBinding](ctx, sdcc.client.Client(), sdc.Namespace), sdcc.eventRecorder, requiredRoleBinding, resourceapply.ApplyOptions{
		ForceOwnership: true,
	})
	if changed {
