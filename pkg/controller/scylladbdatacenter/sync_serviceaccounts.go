...
func (sdcc *Controller) syncServiceAccounts(
	...
	for _, sa := range serviceAccounts {
		...
		err = sdcc.client.Client().Delete(ctx, sa, client.Preconditions{UID: &sa.UID}, client.PropagationPolicy(propagationPolicy))
	...
	_, changed, err := resourceapply.ApplyServiceAccountWithControl(ctx, ctrlclient.ApplyControl[corev1.ServiceAccount](ctx, sdcc.client.Client(), sdc.Namespace), sdcc.eventRecorder, requiredServiceAccount, resourceapply.ApplyOptions{
		ForceOwnership: true,
	})
	if changed {
