...
func (sdcc *Controller) pruneServices(
	...
	for _, svc := range services {
		...
		err = sdcc.client.Client().Delete(ctx, &corev1.PersistentVolumeClaim{
		...
		err = sdcc.client.Client().Delete(ctx, svc, client.Preconditions{
			UID:             &svc.UID,
...
func (sdcc *Controller) syncServices(
	...
	for _, svc := range requiredServices {
		_, changed, err := resourceapply.ApplyServiceWithControl(ctx, ctrlclient.ApplyControl[corev1.Service](ctx, sdcc.client.Client(), sdc.Namespace), sdcc.eventRecorder, svc, resourceapply.ApplyOptions{})
		if changed {
...
func (sdcc *Controller) finishOngoingReplaceNodeUsingHostID(ctx context.Context, sdc *scyllav1alpha1.ScyllaDBDatacenter, svc *corev1.Service, nodeHostID, replacingNodeHostID string) ([]metav1.Condition, error) {
	...
	pod, err := ctrlclient.Get[corev1.Pod](ctx, sdcc.client.Client(), svc.Namespace, svc.Name)
	if err != nil {
	...
	sdc = resolveScyllaDBDatacenterControllerThroughStatefulSet(ctx, sdcc.client.Client(), pod)
	if sdc == nil {
...
func (sdcc *Controller) removePodAndAssociatedPVC(ctx context.Context, sdc *scyllav1alpha1.ScyllaDBDatacenter, svc *corev1.Service) ([]metav1.Condition, error) {
	...
	err := sdcc.client.Client().Delete(ctx, pvcMeta, client.PropagationPolicy(backgroundPropagationPolicy))
	if err != nil {
		if !apierrors.IsNotFound(err) {
	...
	err = sdcc.client.Client().SubResource("eviction").Create(ctx, &corev1.Pod{
		ObjectMeta: metav1.ObjectMeta{
