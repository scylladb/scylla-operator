...
func (sdcc *Controller) getScyllaManagerAgentToken(ctx context.Context, sdc *scyllav1alpha1.ScyllaDBDatacenter) (string, error) {
	...
	secret, err := ctrlclient.Get[corev1.Secret](ctx, sdcc.client.Client(), sdc.Namespace, secretName)
	if err != nil {
...
func (sdcc *Controller) beforeNodeUpgrade(ctx context.Context, sdc *scyllav1alpha1.ScyllaDBDatacenter, sts *appsv1.StatefulSet, ordinal int32, services map[string]*corev1.Service, upgradeContext *internalapi.DatacenterUpgradeContext) (bool, error) {
	...
	err := sdcc.client.Client().Patch(ctx, &corev1.Service{
	...
	pod, err := ctrlclient.Get[corev1.Pod](ctx, sdcc.client.Client(), sdc.Namespace, podName)
	if err != nil {
	...
	err = sdcc.client.Client().Delete(ctx, &corev1.Pod{
		ObjectMeta: metav1.ObjectMeta{
...
func (sdcc *Controller) pruneStatefulSets(
	...
	for _, sts := range statefulSets {
		...
		err := sdcc.client.Client().Delete(ctx, sts, client.Preconditions{UID: &sts.UID}, client.PropagationPolicy(propagationPolicy))
		if err != nil {
...
func (sdcc *Controller) syncRackDecommission(ctx context.Context, sdc *scyllav1alpha1.ScyllaDBDatacenter, rackName string, sts *appsv1.StatefulSet, rackServices map[string]*corev1.Service) ([]metav1.Condition, error) {
	...
	for ordinal := *sts.Spec.Replicas - 1; ordinal >= lowestLeavingOrdinal; ordinal-- {
		...
		err = sdcc.client.Client().Update(ctx, svcCopy)
		if err != nil {
	...
	err = sdcc.client.Client().SubResource("scale").Update(ctx, sts, client.WithSubResourceBody(scale))
	if err != nil {
...
func (sdcc *Controller) syncStatefulSets(
	...
	createdStatefulSets, createProgressingConditions, err := createMissingStatefulSets(
		...
		func(ctx context.Context, required *appsv1.StatefulSet) (*appsv1.StatefulSet, bool, error) {
			return resourceapply.ApplyStatefulSetWithControl(ctx, ctrlclient.ApplyControl[appsv1.StatefulSet](ctx, sdcc.client.Client(), sdc.Namespace), sdcc.eventRecorder, required, resourceapply.ApplyOptions{})
		},
		sdc,
	...
	if ok {
		...
		case internalapi.PreHooksUpgradePhase:
			...
			cm, changed, err := resourceapply.ApplyConfigMapWithControl(ctx, ctrlclient.ApplyControl[corev1.ConfigMap](ctx, sdcc.client.Client(), sdc.Namespace), sdcc.eventRecorder, cm, resourceapply.ApplyOptions{})
			if changed {
		...
		case internalapi.RolloutRunUpgradePhase:
			for _, sts := range requiredStatefulSets {
				...
				err = retry.RetryOnConflict(retry.DefaultBackoff, func() error {
					...
					err = sdcc.client.Client().Update(ctx, freshSts)
					if err != nil {
						return err
					}
		...
		case internalapi.PostHooksUpgradePhase:
			...
			err = sdcc.client.Client().Delete(ctx, cm, client.Preconditions{UID: &cm.UID}, client.PropagationPolicy(metav1.DeletePropagationBackground))
			if err != nil {
