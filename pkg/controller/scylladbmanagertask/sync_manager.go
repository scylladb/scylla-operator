...
func (smtc *Controller) syncManager(
	...
	// TODO: check if has matching annotation
	err = smtc.setScyllaV1TaskStatusAnnotation(ctx, smt, managerTask)
	if err != nil {
		return progressingConditions, fmt.Errorf("can't set scyllav1 task status annotation on ScyllaDBManagerTask %q: %w", naming.ObjRef(smt), err)
	}
	...
	klog.V(2).InfoS("Updating ScyllaDB Manager client task.", "ScyllaDBManagerTask", klog.KObj(smt), "ScyllaDBManagerClientClusterID", clusterID, "ScyllaDBManagerClientTaskType", managerTask.Type, "ScyllaDBManagerClientTaskID", managerTaskID)
	...
	if err != nil {
		...
		klog.V(4).InfoS("Failed to update ScyllaDB Manager client task.", "ScyllaDBManagerTask", klog.KObj(smt), "ScyllaDBManagerClientClusterID", clusterID, "ScyllaDBManagerClientTaskType", managerTask.Type, "ScyllaDBManagerClientTaskName", requiredManagerTask.Name, "ScyllaDBManagerClientTaskID", managerTaskID, "Error", err)
	...
	progressingConditions = append(progressingConditions, metav1.Condition{
		...
		Message:            fmt.Sprintf("Updated a ScyllaDB Manager task: %s (%s).", managerTask.Name, managerTaskID),
	})
...
// The nowUTCFunc parameter is a function that returns the current time in UTC.
func parseStartDate(s string, nowUTCFunc func() time.Time) (strfmt.DateTime, error) {
...
func (smtc *Controller) setScyllaV1TaskStatusAnnotation(ctx context.Context, smt *scyllav1alpha1.ScyllaDBManagerTask, managerClientTask *managerclient.TaskListItem) error {
