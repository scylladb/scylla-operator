...
func Test_makeRequiredScyllaDBManagerClientTask(t *testing.T) {
	...
	tt := []struct {
		...
		},
		{
			...
			overrideOptions: nil,
			expected:        nil,
			expectedErr:     fmt.Errorf("can't make ScyllaDB Manager client schedule: %w", utilerrors.NewAggregate([]error{fmt.Errorf("can't parse start date: %w", fmt.Errorf("can't parse duration: %w", errors.New("time: invalid duration +invalid")))})),
		},
		{
		...
		},
		{
			...
			overrideOptions: nil,
			expected:        nil,
			expectedErr:     fmt.Errorf("can't make ScyllaDB Manager client repair task properties: %w", utilerrors.NewAggregate([]error{fmt.Errorf("can't parse intensity override: %w", &strconv.NumError{Func: "ParseFloat", Num: "invalid", Err: strconv.ErrSyntax})})),
		},
		{
		...
		},
		{
			...
			expected: &managerclient.Task{
				...
				ID:        "",
				Labels: map[string]string{
					"scylla-operator.scylladb.com/managed-hash": "IXCrVx5S94zyp78StL8sU0GPNbUE9AVFGY7meHyjKnuroV3PPvfdt/TBSbxJugmnNlf7gifoo+ftofTWTSgygg==",
					"scylla-operator.scylladb.com/owner-uid":    "uid",
				},
				Name: "repair",
				Properties: map[string]any{
		...
		},
		{
			...
			overrideOptions: nil,
			expected:        nil,
			expectedErr:     fmt.Errorf("can't make ScyllaDB Manager client repair task properties: %w", utilerrors.NewAggregate([]error{fmt.Errorf("can't parse small table threshold override: %w", fmt.Errorf("invalid byte size string %q, it must be real number with unit suffix %q", "invalid size", "B,KiB,MiB,GiB,TiB,PiB,EiB"))})),
		},
	}
