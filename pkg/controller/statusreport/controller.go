...
func NewController(
	...
	observer.AddCachesToSync(podInformer.Informer().HasSynced)
...
func (c *Controller) Sync(ctx context.Context) error {
	...
	if controllerhelpers.HasMatchingAnnotation(pod, naming.NodeStatusReportAnnotation, encodedNodeStatusReportString) {
		klog.V(4).InfoS("Pod already has up-to-date node status report annotation", "Pod", naming.ObjRef(pod))
		return nil
	}
...
func getNodeStatusReport(ctx context.Context) (*internalapi.NodeStatusReport, error) {
	...
	// FIXME: this is copied over from prober, we only need liveness (and snitch DC?) here
