...
// List reads the objects of PT's kind in namespace matching selector through c. The list type comes from c's scheme,
// which is why a List takes a client.Client and not a client.Reader.
func List[T any, PT Object[T]](ctx context.Context, c client.Client, namespace string, selector labels.Selector) ([]PT, error) {
	list, err := newList[T, PT](c)
	if err != nil {
		return nil, err
	}
...
func ListFunc[T any, PT Object[T]](ctx context.Context, c client.Client, namespace string) func(labels.Selector) ([]PT, error) {
...
func newList[T any, PT Object[T]](c client.Client) (client.ObjectList, error) {
	gvk, err := c.GroupVersionKindFor(PT(new(T)))
	if err != nil {
		return nil, fmt.Errorf("can't get GVK for %T: %w", PT(nil), err)
	}

	listGVK := gvk.GroupVersion().WithKind(gvk.Kind + "List")
	obj, err := c.Scheme().New(listGVK)
	if err != nil {
