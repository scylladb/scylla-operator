...
// apply goes through, and a read that observes the caller's own writes, to decide from when the object is missing from
// the caller's cache. The typed implementation reads live for that; a read-your-writes client gives the same for the
// objects the controller wrote.
