...
type managedObjects struct {
	...
	syncCaches func()
	// writes counts the creates and updates the manager has issued so far, the failed ones included.
	writes func() int
	// liveReads counts the reads the manager has made past its caches so far. Nil where the cached reads and the
	// live reads go through the same client.
	liveReads func() int
}
...
// reads and the writes, wrapped in ctrlclient's object controls.
func newControlCertificateManager(t *testing.T, ctx context.Context, keygen ocrypto.KeyGenerator, namespace string) (*CertificateManager, managedObjects) {
	...
	writes := 0
	c := ctrlfake.NewClientBuilder().WithScheme(s).WithInterceptorFuncs(interceptor.Funcs{
	...
	rc := ctrlclient.NewReadYourWritesClient(c)
	cm := NewCertificateManagerWithControl(
		keygen,
		ctrlclient.NewObjectControl[corev1.Secret](ctx, rc),
		ctrlclient.NewObjectControl[corev1.ConfigMap](ctx, rc),
		record.NewFakeRecorder(10),
	)
...
func TestCertificateManager_ManageCertificates(t *testing.T) {
	...
	tt := []struct {
		...
		// cacheLags is whether the manager's cached reads miss what it wrote until syncCaches runs.
		cacheLags bool
	}{
		{
			...
			// The listers see a write once the informer delivers it.
			cacheLags: true,
		},
		{
			name:       "controller-runtime client",
			newManager: newControlCertificateManager,
			// A read-your-writes client's cached reads observe its own writes; the "Read-your-writes client" envtest
			// spec shows that on a real cache.
			cacheLags: false,
		},
	}
	...
	for _, tc := range tt {
		t.Run(tc.name, func(t *testing.T) {
			...
			// run's writes is. The manager has to find them past the snapshot instead of minting a new CA.
			...
			if tc.cacheLags {
				// With the caches still empty, the live read is all that stands between the manager and a second CA:
				// it finds the CA, the apply's cached read doesn't, and the run fails creating the CA that exists
				// instead of minting a new one.
				liveReadsBefore := objects.liveReads()
				err := tryManage()
				if !apierrors.IsAlreadyExists(err) {
					t.Fatalf("expected the run before the caches sync to fail on the existing CA, got %v", err)
				}
				if liveReads := objects.liveReads() - liveReadsBefore; liveReads != 1 {
					t.Errorf("expected the run before the caches sync to read the CA live once, got %d live reads", liveReads)
				}
				if diff := cmp.Diff(first, snapshot()); diff != "" {
					t.Errorf("expected the run before the caches sync to leave the objects unchanged (-first +after):\n%s", diff)
				}
			}
			...
			manage()
			second := snapshot()
			if writes := objects.writes() - writesBeforeSync; writes != 0 {
				t.Errorf("expected the run after the caches sync to issue no writes, got %d", writes)
			}
			if diff := cmp.Diff(first, second); diff != "" {
				t.Errorf("expected the second run to leave the objects unchanged (-first +second):\n%s", diff)
			}
		})
	}
}
