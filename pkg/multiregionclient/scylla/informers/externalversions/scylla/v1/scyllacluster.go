...
import (
	...
	v1 "github.com/scylladb/scylla-operator/pkg/multiregionclient/scylla/listers/scylla/v1"
...
// ScyllaClusterRemoteInformer provides access to a shared informer and lister for
// ScyllaClusters.
type ScyllaClusterRemoteInformer interface {
	Informer() internalinterfaces.UnboundInformer
	Lister() v1.ScyllaClusterRemoteLister
}
...
func NewScyllaClusterInformer(client versioned.RemoteInterface, datacenter, namespace string, resyncPeriod time.Duration, indexers cache.Indexers) cache.SharedIndexInformer {
	...
	return cache.NewSharedIndexInformer(
		&cache.ListWatch{
			ListFunc: func(options metav1.ListOptions) (runtime.Object, error) {
				...
				return c.ScyllaV1().ScyllaClusters(namespace).List(context.TODO(), options)
			},
			WatchFunc: func(options metav1.ListOptions) (watch.Interface, error) {
				...
				return c.ScyllaV1().ScyllaClusters(namespace).Watch(context.TODO(), options)
			},
		},
...
func (f *scyllaClusterRemoteInformer) Informer() internalinterfaces.UnboundInformer {
	return f.factory.UnboundInformerFor(&scyllav1.ScyllaCluster{}, func() internalinterfaces.UnboundInformer{ return &scyllaClusterUnboundInformer{
		factory:          f.factory,
		tweakListOptions: f.tweakListOptions,
	}})
}

func (f *scyllaClusterRemoteInformer) Lister() v1.ScyllaClusterRemoteLister {
	return v1.NewScyllaClusterRemoteLister(f.Informer().Datacenter)
}
...
func (f *scyllaClusterUnboundInformer) Datacenter(datacenter string) cache.SharedIndexInformer {
	return f.factory.InformerFor(&scyllav1.ScyllaCluster{}, datacenter, f.defaultInformer)
}

func (f *scyllaClusterUnboundInformer) HasSynced() bool {
	return f.factory.HasSyncedFor(&scyllav1.ScyllaCluster{})
}

func (f *scyllaClusterUnboundInformer) AddEventHandler(handler cache.ResourceEventHandler) {
	f.factory.AddEventHandlerFor(&scyllav1.ScyllaCluster{}, handler)
}