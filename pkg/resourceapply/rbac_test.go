		...
		t.Run(tc.name, func(t *testing.T) {
			...
			for i := range iterations {
				t.Run("", func(t *testing.T) {
					...
					gotSts, gotChanged, gotErr := ApplyRole(ctx, client.RbacV1(), roleLister, recorder, tc.required, ApplyOptions{})
					if !reflect.DeepEqual(gotErr, tc.expectedErr) {
						t.Fatalf("expected %v, got %v", tc.expectedErr, gotErr)
					}

					if !equality.Semantic.DeepEqual(gotSts, tc.expectedRole) {
						t.Errorf("expected %#v, got %#v, diff:\n%s", tc.expectedRole, gotSts, cmp.Diff(tc.expectedRole, gotSts))
					}

					// Make sure such object was actually created.
					if gotSts != nil {
						createdSts, err := client.RbacV1().Roles(gotSts.Namespace).Get(ctx, gotSts.Name, metav1.GetOptions{})
						if err != nil {
							t.Error(err)
						}
