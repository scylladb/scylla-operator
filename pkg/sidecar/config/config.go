...
// Users are able to change used snitch, and provide their own configuration.
...
func mergeSnitchConfigs(userSnitchConfigPath string, operatorSnitchConfigPath string, scylladbSnitchConfigPath string) error {
	...
	if err == nil {
		...
		userDCSuffix = userProperties.GetString("dc_suffix", "")
	}
	...
	if len(userDCSuffix) > 0 {
		_, _, err = operatorProperties.Set("dc_suffix", userDCSuffix)
		if err != nil {
			return fmt.Errorf("can't set dc_suffix: %w", err)
		}
	}
	...
	_, err = operatorProperties.Write(snitchConfigFile, properties.UTF8)
	if err != nil {
