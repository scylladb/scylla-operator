...
- A regional GKE cluster provisioned with a dedicated ScyllaDB node pool with local NVMe SSDs, spread across 3 zones.
  If you do not have one yet, follow [Set up a GKE cluster for ScyllaDB](https://operator.docs.scylladb.com/stable/install-operator/provision-infrastructure/set-up-gke-cluster.md).
...
- The environment variables from the [GKE cluster setup guide](https://operator.docs.scylladb.com/stable/install-operator/provision-infrastructure/set-up-gke-cluster.md#set-environment-variables) exported in your shell (at minimum `GCP_REGION`, `GKE_ZONE_1`, `GKE_ZONE_2`, `GKE_ZONE_3`).
...
Follow [Configure nodes](https://operator.docs.scylladb.com/stable/deploy-scylladb/before-you-deploy/configure-nodes.md), selecting the **GKE** tab where applicable, then return here to deploy the cluster.
...
To tear down the GKE infrastructure, follow the [Clean up](https://operator.docs.scylladb.com/stable/install-operator/provision-infrastructure/set-up-gke-cluster.md#clean-up) section of the GKE cluster setup guide.
