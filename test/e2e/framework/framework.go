...
func NewFramework(namePrefix string) *Framework {
	...
	f := &Framework{
		...
		e2eArtifactsDir: e2eArtifactsDir,

		workerClusters: map[string]*Cluster{},
	}

	f.cluster = newCluster(f.name, f.e2eArtifactsDir, TestContext.RestConfig, f.CommonLabels())
	...
	for workerClusterKey, workerClusterRestConfig := range TestContext.WorkerRestConfigs {
		...
		f.workerClusters[workerClusterKey] = newCluster(workerClusterName, f.e2eArtifactsDir, workerClusterRestConfig, f.CommonLabels())
	}
...
func newCluster(name, baseArtifactsDir string, restConfig *restclient.Config, namespaceLabels map[string]string) *Cluster {
	artifactsDir := ""
	if len(baseArtifactsDir) != 0 {
		artifactsDir = path.Join(baseArtifactsDir, name)
	...
	return NewCluster(
		...
		func(ctx context.Context, adminClient kubernetes.Interface, adminClientConfig *restclient.Config) (*corev1.Namespace, Client) {
			return CreateUserNamespace(ctx, name, namespaceLabels, adminClient, adminClientConfig)
		},
	)
}
