...
var _ = g.Describe("NodeConfig Optimizations", framework.Serial, func() {
	...
	g.It("should set Scylla process nofile rlimit to maximum", func() {
		...
		pod, err := f.KubeClient().CoreV1().Pods(sc.Namespace).Get(ctx, podName, metav1.GetOptions{})
		o.Expect(err).NotTo(o.HaveOccurred())

		stdout, stderr, err := executeInPod(f.ClientConfig(), f.KubeClient().CoreV1(), pod, "bash", "-c", `prlimit --pid=$(pidof scylla) --nofile --noheadings --output=SOFT`)
		...
		stdout, stderr, err = executeInPod(f.ClientConfig(), f.KubeClient().CoreV1(), pod, "bash", "-c", `prlimit --pid=$(pidof scylla) --nofile --noheadings --output=HARD`)
		o.Expect(err).NotTo(o.HaveOccurred(), stdout, stderr)
