...
var _ = g.Describe("MultiDC cluster", framework.MultiDatacenter, func() {
	fs := framework.NewFrameworkSet("scyllacluster")

	g.It("should form when external seeds are provided to ScyllaClusters", func() {
		// TODO: should the test actually check this? It feels like it shouldn't test the infrastructure.
		o.Expect(len(fs)).To(o.BeNumerically(">=", 2))
		f1, f2 := fs[0], fs[1]
