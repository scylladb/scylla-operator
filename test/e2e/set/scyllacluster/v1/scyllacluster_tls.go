	...
	g.It("should setup and maintain up to date TLS certificates", func() {
		...
		for _, tc := range []struct {
			...
			func() {
				...
				framework.By("Waiting for the ScyllaCluster to rollout (RV=%s)", sc.ResourceVersion)
				waitCtxL1, waitCtxL1Cancel := v1utils.ContextForRollout(ctx, sc)
				defer waitCtxL1Cancel()
				sc, err = v1utils.WaitForScyllaClusterState(waitCtxL1, f.ScyllaClient().ScyllaV1(), sc.Namespace, sc.Name, utils.WaitForStateOptions{}, v1utils.IsScyllaClusterRolledOut)
				o.Expect(err).NotTo(o.HaveOccurred())
				...
				nodeClusterIPs, nodeIDs, err := v1utils.GetHostsAndUUIDs(ctx, f.KubeClient().CoreV1(), sc)
				o.Expect(err).NotTo(o.HaveOccurred())
	...
	g.It("should rotate TLS certificates before they expire", func() {
		...
		for _, item := range items {
			func() {
				...
				waitCtxL1, waitCtxL1Cancel := context.WithTimeout(ctx, v1utils.SyncTimeout)
				defer waitCtxL1Cancel()
				...
				if len(item.cmName) != 0 {
					...
					waitCtxL2, waitCtxL2Cancel := context.WithTimeout(ctx, v1utils.SyncTimeout)
