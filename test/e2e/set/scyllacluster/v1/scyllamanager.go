...
var _ = g.Describe("Scylla Manager integration", func() {
	...
	g.It("should discover cluster and sync tasks", func() {
		...
		waitCtx2, waitCtx2Cancel := v1utils.ContextForManagerSync(ctx, sc)
		defer waitCtx2Cancel()
		sc, err = v1utils.WaitForScyllaClusterState(waitCtx2, f.ScyllaClient().ScyllaV1(), sc.Namespace, sc.Name, utils.WaitForStateOptions{}, registeredInManagerCond, repairTaskScheduledCond, backupTaskSyncFailedCond)
		o.Expect(err).NotTo(o.HaveOccurred())

		managerClient, err := v1utils.GetManagerClient(ctx, f.KubeAdminClient().CoreV1())
		o.Expect(err).NotTo(o.HaveOccurred())
