...
var _ = g.Describe("ScyllaDBCluster webhook", func() {
	...
	g.It("should forbid invalid requests", func() {
		...
		if framework.TestContext.ScyllaClusterOptions != nil {
			if framework.TestContext.ScyllaClusterOptions.ExposeOptions.NodeServiceType == scyllav1.NodeServiceTypeClusterIP ||
				framework.TestContext.ScyllaClusterOptions.ExposeOptions.ClientsBroadcastAddressType == scyllav1.BroadcastAddressTypeServiceClusterIP ||
				framework.TestContext.ScyllaClusterOptions.ExposeOptions.NodesBroadcastAddressType == scyllav1.BroadcastAddressTypeServiceClusterIP {
				g.Skip("ScyllaDBCluster doesn't support exposure via ClusterIP, it won't pass validation")
			}
		}
