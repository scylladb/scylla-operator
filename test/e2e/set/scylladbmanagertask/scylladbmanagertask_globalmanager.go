...
var _ = g.Describe("ScyllaDBManagerTask integration with global ScyllaDB Manager", func() {
	...
	g.It("should synchronise a repair task for ScyllaDBDatacenter", func(ctx g.SpecContext) {
		...
		o.Eventually(func(eo o.Gomega, ctx context.Context) {
			...
			eo.Expect(managerTask.Properties.(map[string]interface{})["parallel"].(json.Number).Int64()).To(o.Equal(*smt.Spec.Repair.Parallel))
		...
		o.Eventually(func(eo o.Gomega, ctx context.Context) {
			// TODO: issue ref
			// 'GetTask' is broken and does not return an error when the task has been deleted.
		...
		}).WithContext(ctx).WithTimeout(30 * time.Second).WithPolling(3 * time.Second).Should(o.Succeed())
