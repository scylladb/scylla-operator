...
var _ = g.Describe("ScyllaDBManagerTask integration with global ScyllaDB Manager", func() {
	...
	g.DescribeTable("should synchronise a backup task for ScyllaDBDatacenter and support a manual restore procedure", func(ctx g.SpecContext, e entry) {
		...
		verifyRestoreTaskCompletion := func(eo o.Gomega, ctx context.Context, restoreTaskID string) {
			restoreProgress, err := managerClient.RestoreProgress(ctx, targetManagerClusterID, restoreTaskID, "latest")
			eo.Expect(err).NotTo(o.HaveOccurred())

			o.Expect(restoreProgress.Errors).To(o.BeEmpty())
			o.Expect(restoreProgress.Run).NotTo(o.BeNil())
			o.Expect(restoreProgress.Run.Status).To(o.Equal(managerclient.TaskStatusDone))
		}

		framework.By("Waiting for the schema restore task to finish")
		schemaRestoreCompletionCtx, schemaRestoreCompletionCtxCancel := context.WithTimeout(ctx, utils.SyncTimeout)
		defer schemaRestoreCompletionCtxCancel()
		o.Eventually(verifyRestoreTaskCompletion).
			WithContext(schemaRestoreCompletionCtx).
		...
		tablesRestoreCompletionCtx, tablesRestoreCompletionCtxCancel := context.WithTimeout(ctx, utils.SyncTimeout)
		defer tablesRestoreCompletionCtxCancel()
		o.Eventually(verifyRestoreTaskCompletion).
			WithContext(tablesRestoreCompletionCtx).
			WithTimeout(10 * time.Minute).
			WithPolling(5 * time.Second).
