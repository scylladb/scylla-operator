...
func Verify(ctx context.Context, sc *scyllav1alpha1.ScyllaDBCluster, rkcClusterMap map[string]framework.ClusterInterface) {
	...
	o.Expect(sc.Status.Conditions).To(o.ConsistOf(func() []interface{} {
		...
		condList := []condValue{
			...
			{
				condType: internalapi.MakeKindControllerCondition("ConfigMap", scyllav1alpha1.ProgressingCondition),
				status:   metav1.ConditionFalse,
			},
			{
				condType: internalapi.MakeKindControllerCondition("ConfigMap", scyllav1alpha1.DegradedCondition),
				status:   metav1.ConditionFalse,
			},
		}
