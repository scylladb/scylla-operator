...
// withInformerLag lags the informer of obj's kind further behind the API server than the default.
func withInformerLag(lag time.Duration, obj client.Object) func(*ctrlcache.Options) {
	return func(o *ctrlcache.Options) {
		if o.ByObject == nil {
			o.ByObject = map[client.Object]ctrlcache.ByObject{}
		}
		// A per-kind transform replaces the default one, so the default lag is added back.
		o.ByObject[obj] = ctrlcache.ByObject{
			Transform: informerLagTransform(scyllaDBDatacenterControllerDefaultInformerLag + lag),
		}
	}
}
...
var _ = g.Describe("ScyllaDBDatacenter controller with lagging informers", func() {
	...
	g.It("should not report a rack as rolled out from a StatefulSet cache that has not observed the applied rollout", func(ctx g.SpecContext) {
		...
		runScyllaDBDatacenterController(ctx, env, withInformerLag(informerLag, &appsv1.StatefulSet{}))
	...
	g.It("should not scale a rack up over a node whose requested decommission has not been observed yet", func(ctx g.SpecContext) {
		...
		runScyllaDBDatacenterController(ctx, env, withInformerLag(informerLag, &corev1.Service{}))
