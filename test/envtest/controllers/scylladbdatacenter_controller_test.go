...
var _ = g.Describe("ScyllaDBDatacenter controller", func() {
	...
	g.Describe("decommissioning nodes record", func() {
		...
		setupRacks := func(ctx g.SpecContext, rackNames []string, nodes int32, mutators ...func(*scyllav1alpha1.ScyllaDBDatacenter)) *scyllav1alpha1.ScyllaDBDatacenter {
			g.GinkgoHelper()
			...
			sdc := makeEnvtestScyllaDBDatacenter(env.Namespace(), rackNames, append([]func(*scyllav1alpha1.ScyllaDBDatacenter){withRackTemplateNodes(nodes)}, mutators...)...)
			sdc, err := env.ScyllaClient().ScyllaV1alpha1().ScyllaDBDatacenters(env.Namespace()).Create(ctx, sdc, metav1.CreateOptions{})
			o.Expect(err).NotTo(o.HaveOccurred())
		...
		g.DescribeTable("scaling another rack while a rack has a node decommissioning",
			func(ctx g.SpecContext, enableParallelNodeOperations bool) {
				const otherRackName = "rack-b"

				sdc := setupRacks(ctx, []string{rackName, otherRackName}, initialNodes, withEnableParallelNodeOperations(enableParallelNodeOperations))
				rackStatefulSetName := naming.StatefulSetNameForRack(sdc.Spec.Racks[0], sdc)
				leavingServiceName := naming.MemberServiceName(sdc.Spec.Racks[0], sdc, int(initialNodes-1))
			...
			g.Entry("keeps scaling the other rack with parallel node operations enabled", true),
			g.Entry("waits for the decommission with parallel node operations disabled", false),
		)
...
func scaleRack(ctx context.Context, e *envtest.Environment, sdcName, rackName string, nodes int32) {
	...
	_, rackIdx, ok := oslices.Find(sdc.Spec.Racks, func(rack scyllav1alpha1.RackSpec) bool {
		return rack.Name == rackName
	})
	o.Expect(ok).To(o.BeTrue(), "rack %q not found in ScyllaDBDatacenter %q", rackName, naming.ObjRef(sdc))

	// Patch instead of update: the controller keeps updating the status, so an optimistic update is prone to conflicts.
