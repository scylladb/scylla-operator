...
- **Scale up** — new pods are appended at the end of the ordinal sequence (highest index).
  After the new node joins the token ring, the Operator automatically triggers a [data cleanup](https://operator.docs.scylladb.com/v1.21/understand/automatic-data-cleanup.md) on affected nodes.
...
If a specific node is unhealthy, use [node replacement](https://operator.docs.scylladb.com/v1.21/operate/replace-nodes.md) instead.

For background on the StatefulSet-per-rack architecture, see [StatefulSets and racks](https://operator.docs.scylladb.com/v1.21/understand/statefulsets-and-racks.md).
...
- [StatefulSets and racks](https://operator.docs.scylladb.com/v1.21/understand/statefulsets-and-racks.md) — how StatefulSets map to racks and why mid-set removal is not possible
- [Replace nodes](https://operator.docs.scylladb.com/v1.21/operate/replace-nodes.md) — replacing a specific unhealthy node without scaling
- [Migrate a rack to a new node pool](https://operator.docs.scylladb.com/v1.21/operate/migrate-rack-to-new-node-pool.md) — scaling up a new rack and scaling down the old one to migrate infrastructure
- [Perform a rolling restart](https://operator.docs.scylladb.com/v1.21/operate/perform-rolling-restart.md) — restarting all nodes without changing the cluster size
