...
- Waits for [ignition](https://operator.docs.scylladb.com/v1.21/understand/ignition.md) before starting — it does not run until ScyllaDB itself is ready.
...
- **Restore** is not yet available through the Operator’s declarative API. To restore from a Manager backup, you must exec into the Manager pod and use `sctool` directly. See [Back up and restore](https://operator.docs.scylladb.com/v1.21/operate/back-up-and-restore.md).
