...
Replacement works on any ordinal because the pod identity and PVC are tied to the ordinal — the StatefulSet recreates a pod with the same name at the same position. See [Replace nodes](https://operator.docs.scylladb.com/v1.21/operate/replace-nodes.md) for the step-by-step procedure.

### Maintenance mode

Setting the `scylla/node-maintenance` label on a member Service puts that node into maintenance mode. The Operator reconfigures the Service so that traffic is no longer routed to the node, allowing you to perform manual operations on it without affecting client traffic. See [Maintenance mode](https://operator.docs.scylladb.com/v1.21/operate/use-maintenance-mode.md).
...
To diagnose, check the logs of the failing pod and its events. If the problem is with the new configuration, fix the spec and the rollout resumes automatically. If the pod is stuck for other reasons (crash loop, resource issues), see [Investigate pod restarts](https://operator.docs.scylladb.com/v1.21/troubleshoot/investigate-restarts.md).

For log-level changes on a stuck cluster where a rolling restart is not feasible, see [Changing log level](https://operator.docs.scylladb.com/v1.21/troubleshoot/change-log-level.md).

### Deleting a StatefulSet

Deleting a StatefulSet with the default cascade policy also deletes all its pods (but not PVCs). If the Operator is running, it immediately recreates the StatefulSet, which recreates the pods. Using `--cascade=orphan` preserves the running pods — this is used during advanced recovery procedures such as [Recovering from a failed replace](https://operator.docs.scylladb.com/v1.21/troubleshoot/recover-from-failed-replace.md).

## Related pages

- [Pod disruption budgets](https://operator.docs.scylladb.com/v1.21/understand/pod-disruption-budgets.md) — how PDBs interact with rolling updates and scale-down.
- [Sidecar](https://operator.docs.scylladb.com/v1.21/understand/sidecar.md) — what runs inside each pod and how decommission is driven.
- [Scaling](https://operator.docs.scylladb.com/v1.21/operate/scale-add-remove-racks.md) — how-to guide for scaling operations.
- [Replacing nodes](https://operator.docs.scylladb.com/v1.21/operate/replace-nodes.md) — how-to guide for node replacement.
- [Changing log level](https://operator.docs.scylladb.com/v1.21/troubleshoot/change-log-level.md) — emergency changes when a rollout is stuck.
