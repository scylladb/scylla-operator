...
Performance tuning is enabled when you create a [NodeConfig](https://operator.docs.scylladb.com/stable/deploy-scylladb/before-you-deploy/configure-nodes.md) resource that targets your ScyllaDB nodes.
...
Both node-level and container-level tuning **block ScyllaDB startup** via the [ignition](https://operator.docs.scylladb.com/stable/understand/ignition.md) mechanism.
NodeConfigs that have not completed tuning for a given container are added to the blocking list in the per-pod ConfigMap, preventing the ignition signal from being created until all tuning — including node-level — has finished.
...
Container-level tuning **blocks ScyllaDB startup via the [ignition](https://operator.docs.scylladb.com/stable/understand/ignition.md) mechanism**.
...
For a step-by-step guide on configuring CPU pinning, see [CPU Pinning](https://operator.docs.scylladb.com/stable/deploy-scylladb/before-you-deploy/configure-cpu-pinning.md).
